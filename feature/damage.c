...
//					sprintf("%s被干掉了。", this_object()->name(1)));
	...
	if ( me->query_temp("again_death") != "yes")
	{
		if ( stringp(killer) )
			CHANNEL_D->do_channel(rum_ob, "rumor",
    ...
    if ( userp(this_object()) ) {
       if (stringp(query_temp("last_eff_damage_from")) ) {
           ...
           log_file("PLAYER_DEATH", sprintf("%s(%s) 莫名其妙地死亡 on %s。\n", me->query("name"), 
...
//                if( this_object()->query("family/master_id")=="feng qingyang" )
//                        CHAR_D->break_relation(this_object()); //added by Qfy,discard by Karlopex@sz.
                if ( intp(this_object()->query("clbscore") ) && !stringp(this_object()->query("clbpos")))
		{	left = this_object()->query("clbscore")/100*5;
                        if ( left > 1000 )      left = 1000;
			left = this_object()->query("clbscore") - left;
			this_object()->set("clbscore",left);
			this_object()->save();
		}          //add by Yujie
		
		this_object()->delete_temp("last_damage_from",1);//added by Lisser
		this_object()->delete_temp("last_eff_damage_from",1);//added by Lisser
