...
int main(object me, string arg)
{
        ...
        if ( userp(tob) && ! wizardp(tob) && wizhood(me) != "(arch)" && wizhood(me) != "(wizard)"&& wizhood(me) != "(admin)")
                return notify_fail("只有天神才能给普通玩家复制武功。\n");

        if (ob == tob)
                return notify_fail("不需要复制吧。\n");

        log_file("static/copyskill", sprintf("[%s] %s(%s) copy %s(%s) 的功夫给 %s(%s).\n",
                  ctime(time())[0..15], me->name(1), me->query("id"),ob->name(1), ob->query("id"),tob->name(1), tob->query("id")));
