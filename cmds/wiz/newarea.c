...
{  string filename,file;
   ...
   if (me->query("id") != "buwu") return notify_fail("这件事情只能布武来做!!\n");
   if(room->query("coor/z")!=0) return notify_fail("你只能扩展神州大街！！\n");
   if(room->query("coor/x")!=50) return notify_fail("你只能扩展神州大街！！\n");
   if(room->query("coor/y")<30) return notify_fail("你只能扩展神州大街！！\n");
...
filename=__ROOMDIR__"street"+(string)(i-1)+".c";file=read_file(filename);
   ...
   message_vision("嘁哩嚓哪还真出来一片空地，神州大街看起来又长了一些。\n",me);
return 1;
}
