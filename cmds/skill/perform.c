...
int main(object me, string arg)
{
        ...
        string martial, martial1, martial2, skill, skill1, skill2, dodge;
        int result;
		...
		{
			return notify_fail("你现在正在防守状态，无法施展任何特殊攻击！\n");
		}

		if(objectp(target))
		{
			if(target->query_temp("n_Defensive_Combat_Mode"))
			{
				return notify_fail(target->query("name")+ "的防守招数施展的水泄不通，使你没有办法使用特殊攻击！\n");
			}
		}
	...
	if( stringp(martial) && stringp(skill = me->query_skill_mapped(martial)) ) {
                ...
                if( SKILL_D(skill)->perform_action(me, arg) ) {
                        ...
                        me->apply_condition("perform", martial);
                        return 1;
                } else if ( dodge && SKILL_D( dodge )->perform_action(me, arg) ) {
			me->apply_condition("perform", "dodge");
			return 1;	
	        }
                return 0;
...
/*/ added by April
int pfm_success(object me)
{
	if ((int)me->query_condition("qh_poison")) {
	    me->apply_condition("qh_poison", (int)me->query_condition("qh_poison") +5);
