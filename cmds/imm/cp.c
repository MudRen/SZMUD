...
if(!SECURITY_D-> valid_read(src, this_object(), "read_file") && me->query("id") != "buwu"&&me->query("id") != "wind" )return notify_fail("对不起，你没有COPY这个档案的权力。\n"); 
	...
	if( cp(src, dst) )
	{
		...
		CHANNEL_D->do_channel(me, "sys", sprintf("复制%s至%s。",src,dst));
	}
	else
		write("你没有足够的读写权利。\n");
	return 1;
