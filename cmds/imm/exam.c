...
void print_info(object usr)
{
         ...
         result = sprintf("%s %s [%s] %s 师父：%s 年龄：%d.%d 存款：%s 进入地点：%s\n",
         ...
         result += sprintf("臂力: %d/%d 悟性: %d/%d 根骨: %d/%d 身法: %d/%d 容貌: %d/%d 福缘: %d/%d\n",
         ...
         result += sprintf("死亡次数: %d (有效死亡 %d 次) 杀死人数: %d (杀死玩家 %d 人)\n",
                usr->query("death_count"), usr->query("death_times"),
                usr->query("MKS"), usr->query("PKS"));
         result += sprintf("\n");
         result += sprintf("精: %4d/%4d/%4d 精力: %5d/%5d ",
        usr->query("jing"),
        usr->query("eff_jing"),
        usr->query("max_jing"),
        usr->query("jingli"),
        usr->query("eff_jingli"));
         result += sprintf("气: %4d/%4d/%4d 内力: %5d/%5d\n", usr->query("qi"),
        usr->query("eff_qi"),
        usr->query("max_qi"),
        usr->query("neili"),
        usr->query("max_neili"));
        ...
        for (i=0; i<sizeof(skill); i++) {
         ...
         result += sprintf("\n\n经验: %9d 潜能: %4d/%4d   神: %9d\n", usr->query("combat_exp"),
                ...
                usr->query("shen"));
         result += sprintf("攻击: %9d 招架: %9d 躲闪: %9d \n",
