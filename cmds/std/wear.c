...
int do_wear(object me, object ob)
{
	...
	if( ob->query("tengjia")
	  &&  (string) me->query("family/family_name")!="大理段家" && !wizardp(me))
	      return notify_fail("你非大理弟子怎么好意思穿用人家的东西！\n" );
	if( ob->query("ruanjia")
	  &&  (string) me->query("title")!="大理国御林军都督" &&!wizardp(me)) 
	      return notify_fail("软甲乃大理国宝，只有大理王侯才可以使用！\n" );
