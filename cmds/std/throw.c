...
int main(object me, string arg)
{
	...
	if (ob->query("no_drop")) return notify_fail("这个东西不能离开你。\n");
	if (!(victim = present(who, environment(me)))) return notify_fail("这里没有你的目标。\n");
	...
	if( (time < 3600) && userp(victim)) return notify_fail("你自觉杀心太重，手软了下来！\n");

	if (victim->query("winner")) return notify_fail("不可对"+victim->short()+"放肆！\n");
...
//	ap = random(ap);
	bili += me->query("jiali")/3;		//大幅度增加伤害 add by April
	ap = random(ap*2);	//扩大成功率 add by April
