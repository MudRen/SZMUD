...
///////////////////////////////////////////////////////////////////////////
       ...
       if( time < 3600 && obj->query("canScheme") && !wizardp(me)) return notify_fail(HIG"你一身杀气，那个地方你进不去。\n"NOR);

        if(!me->query("newjob/dwelling")
         && obj->query("owner") == me->query("id")
         && obj->query("short")[7..14] == "神州大街")
        {
                tell_object(me,HIG"您的住址暂时定义为：”"+obj->query("short")[15..<8]+"“。\n"NOR);
                ...
                if( me->query("race") == "人类" ) {
                        if( env->query("outdoors") ) {
                                if( objectp(me->query("rided")) ) {
                                        ...
                                        mout = "急步往" + dir + "离开。\n";
                                                                                min = "手拿"+thing->query("name")+"快步走了过来。\n";}
                                                                        else{
                                        mout = "急步往" + dir + "离开。\n";
                                                                                min = "快步走了过来。\n";}
