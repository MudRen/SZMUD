...
int isPlayer( object obj )
{
    if ( interactive( obj ) ) 
        return 1;
    if ( environment( obj ) && obj->query( "scheming" ) )
        return 1;
    return 0;
}
...
mixed main(object me, string arg, int remote)
{
    ...
    str = sprintf(CYN"『" + INTERMUD_MUD_CNAME + "』"HIW"目前在线的%s有："NOR"\n",opt_wiz?"巫师":"玩家");
    str += "─────────────────────────────────────\n";
    ...
    if( opt_long || opt_wiz ) 
    {
        ...
        while( j-- ) 
        {
            ...
            else if ( interactive( list[j] ) )
            {	    
                str = sprintf("%s%12s%s%s\n",
                        str, RANK_D->query_rank(list[j]),
                        list[j]->short(1),
                        query_idle(list[j]) > 120?HIM "<发呆中>" NOR:" ");
            }
    ...
    else 
    {
        ...
        msgs = HIM"粉红色"NOR" 表示发呆中的使用者。";
        if( ppl_cnt%5 ) str += "\n";
    }
    str += "─────────────────────────────────────\n";
    str = sprintf("%s共有%s位使用者连线中，系统负担：%s%s\n", str, chinese_number(total+20),
    query_load_average()+"\n",msgs);
...
int filter_user(object user, int family)
{
    string fam = user->query("family/family_name");
        ...
        case "少林派": return family & O_SHAOLIN; break;
         ...
         case "昆仑派": return family & O_KUNLUN; break;
...
范例：who, who -w, who -i -wudang -emei, who -l -shaolin @opn
