...
void view_main()
{
     ...
     else if( select== "depart") name = "巫师通告";
     else if( select== "map") name = "地形指南";
     else if( select== "user") name = "个人设定";
     else if( select== "board") name = "论坛精华";
     else if( select== "post") name = "灌水天地";
//     else if( select== "leaveword") name= "各地留言";
     else if( select== "culture" ) name="门派介绍";
     else if( select== "other" ) name="其他内容";
     else if( select== "xyz" ) name="好好学习";     
//   write(YEL"分类选单"WHT"                     【"+MUDLIB_NAME+"】                  "YEL"选单["+select+"]\n"NOR);     
     write(HIY"分类选单"HIC"                 【"+MUD_NAME+"江湖论坛】            "HIY"选单["+select+"]\n"NOR);     
...
void m_display()
 {
     ...
     view_main();
     write("                        "BRED"(I)"NOR"ntro  　〖  游戏概况  〗\n");
     write("                        "BRED"(D)"NOR"epart 　〖  巫师通告  〗\n");
     write("                        "BRED"(M)"NOR"ap    　〖  地形指南  〗\n");
     write("                        "BRED"(U)"NOR"ser     〖  个人设定  〗\n");
     write("		        "BRED"(C)"NOR"ulture  〖  门派介绍  〗\n");
     write("                        "BRED"(B)"NOR"board 　〖  论坛精华  〗\n");
//     write("                        "BRED"(L)"NOR"eaveword〖  各地留言  〗\n");
     write("     		        "BRED"(O)"NOR"ther    〖  其他内容  〗\n");	
     write("                        "BRED"(X)"NOR"yz      〖  好好学习  〗\n"); 
     write("	                "BRED"(P)"NOR"BBS     〖  灌水天地  〗\n");   	
     write("                        "BRED"(G)"NOR"oodbye　【  "HIM"回到江湖"NOR"  】\n");
     view_main_1();
}
...
int message1()
{
      ...
      write(HIM"      亲爱的"HIR+me->name()+HIM"啊, 您在【神州】的江湖中已经摸爬滚打了"+age_string((int)me->query("mud_age"))+"耶!!\n");
...
/*int wizlist()
{
        string *wizlist,*list,wiz_name,wiz_level,str,str1,str2,str3;
        int i,s1,s2,s3;
        str1="",str2="",str3="";
        s1=0;s2=0;s3=0;
        ...
        write(HIR"   【 天 仙 】: "NOR+str1+"\n");
        write(HIY"   【 神 仙 】: "NOR+str2+"\n");
        write(HIC"   【 天 帝 】: "NOR+str3+"\n");
}*/
...
void view3()
{
	...
	{
		w=explode(text[0],"==@==");
		file+=sprintf(CYN" ("BBLU +GRN"%2d"NOR +CYN")"WHT"【%|16s】"NOR,i+1,w[0]);
		if(i%3==2) file+="\n";
		text-=({text[0]});
		i++;
	}
...
/*void view6()
{
	...
	if(file_size(BOARDCENTER)>-1)
	{
		...
		{			
			...
			name=name[0..15];
			destruct(ob);
			msg=sprintf(" 『"CYN +BBLU+"%2d"NOR+"』"+WHT+"【%|16s】"NOR,i+1,name);		
			str+=msg;
			write(msg);
			++j;
			if(j==3) 
		...
		str+=sprintf(" 『"CYN +BBLU+"%2s"NOR+"』"+WHT+"【%|16s】\n"NOR,"B","返回主菜单");		
		printf(" 『"CYN +BBLU+"%2s"NOR+"』"+WHT+"【%|16s】\n"NOR,"B","返回主菜单");	
