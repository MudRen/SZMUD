...
{       
        ...
        {
                tell_object(me,"丁春秋点了点头说道：“幸亏你把阿紫捉回来，神木王鼎才得以失而复返，你既然需要就拿去吧。”\n");
...
int accept_object(object who, object ob){
        ...
        if(ob->query("id")=="a zi"){

                {
                        ...
                        tell_object(me,"门外进来两名星宿弟子把阿紫拖出去了。\n");
                              CHANNEL_D->do_channel(this_object(), "menpai","太好了！多亏了"+me->query("name")+"，我派至宝才得以失而复返！");
                         CHANNEL_D->do_channel(this_object(), "rumor", "听说星宿派找回了丢失的宝贝。");        
                         tell_object(me,"丁春秋赞许地拍了拍你的头。\n");
                        "/adm/daemons/xxquestd.c"->init_quest();        
                        pot = random(600)+80;
                        exp = 1200 + random( 20 );
                        me->add("combat_exp", exp);
                        me->add("potential", pot);
                        if(me->query("potential")>me->query("max_potential"))
                                me->set("potential",me->query("max_potential"));
                        tell_object(me,"你得到了"+chinese_number(exp)+"点经验值，和"+chinese_number(pot)+"点潜能。\n");
                        me->set("ask/ding", 1);
                     call_out("destob",1,ob);
                        return 1;
                }
        ...
        if(ob->query("id")=="noname book")
        {
                ...
                {
                        tell_object(me,"这……这是易筋经残篇！！
好徒儿，咱们星宿一统江湖的日子来了！此书既然由你取得，日后你当然也可
