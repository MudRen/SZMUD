...
"ÇëÁË£¬»¶Ó­À´µ½ÑøÂí³¡£¡\n");
  ...
  command("say 
ÄãÊÇÊ²Ã´ÈË£¬Ë½´³»Ê¼ÒÖ®µØ£¬ÄãÏëÍµÂí²»³É£¡");
}
...
int do_walkhorse()
{
        ...
        if ( myfam && myfam["family_name"] != "´óÀí¶Î¼Ò" )
           {
     command("say
ÕâÐ©¶¼ÊÇÍõÒ¯µÄ°®Âí£¬ÎÒ¿É²»·ÅÐÄ½Ì¸øÍâÈË£¡");
 return 1;
        }
        ...
        if ( ob->query("djob/supervisor") )
        {
                ...
                if ( ob->query("djob/supervisor") == "Âí·ò") 
                {
                        ...
                        command("say 
Èç¹ûÄã¼±×ÅÈ¥Á·¹¦£¬Äã¿ÉÒÔ¸úÎÒËµÏëÏÈ×ß¡£");
                } else  
                        command("say 
Èç¹ûÄãÏëÏÈ×ß£¬¿ÉÒÔÖ±½ÓÈ¥¸æËß"+ob->query("djob/supervisor"));        
        ...
        ob->set_temp("apply/short", 
                ({ HIW" 
´óÀíÆ½ÄÏÍõ¸®Âí·ò"NOR+ob->query("name")+"("+capitalize(ob->query("id"))+")"}));
