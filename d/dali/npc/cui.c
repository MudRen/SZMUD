...
ob->add("balance", wage);
if ( ob->query( "balance" ) > 10000000 )
    ob->set( "balance", 10000000 );
return ("加油继续干！");
...
wage 
= 
(ob->query("dali/jobdone")-ob->query("lastcheck"))*ob->query("combat_exp") 
/700; // by mariner, dali salary is too much, 6gold/job for romance
ob->set("lastcheck",ob->query("dali/jobdone"));
command("say 这是你打杂的酬劳，好好干！");
command("say 不过我看你笨手笨脚的，给你钱也收不牢，干脆我帮你存下吧！");
ob->add("balance", wage);
