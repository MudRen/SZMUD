...
string ask_me()
{
	...
	ob->set("combat_exp",me->query("combat_exp")*(150+random(51))/100);
	...
	ob->set("max_neili", me->query("max_neili")*(9+random(3))/10);
	...
	call_out("moveto", 15+random(7), ob, room_no);
	...
	return "你去那一带巡查巡查，发现有骚扰治安的，抓住送定安府关押。";
}
