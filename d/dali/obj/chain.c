...
int do_arrest(string arg)
{
	...
	if (!arg)
        return notify_fail("你要锁捕谁？\n");

	if (environment(me)->query("no_fight"))
		return notify_fail("这里不准战斗。\n");

	if (!objectp(weapon=me->query_temp("weapon")) || weapon->query("id")!="tie lian")
		return notify_fail("你拿什么套？\n");
	...
	if (userp(victim))
		return notify_fail("你不能锁捕玩家。\n");

	if (!living(victim))
		return notify_fail("这不是活物！\n");

	if ((int)me->query_temp("marks/arrestee"))
		return notify_fail("你上个要抓的人还没抓到，不要多事了！\n");

	if (strsrch(file_name(environment(me)), "/d/dali/") == -1)
		return notify_fail("这里不是大理境内，轮不到你来多事！\n");
	...
	if (fighter == me)
		return 0;
	if (!victim->is_killing(fighter->query("id")))
		return notify_fail("此人非肇事者。\n");

	if (fighter->query("qi") > victim->query("qi"))
		return notify_fail("他们只是在切搓武功。\n");

	message_vision(HIW"$N高喊一声：大理境内休得撒野！走！跟我见官去！\n"NOR, me);
...
{
	if( victim->query("combat_exp",1)>50)
		return HIY "铁链呛啷啷的往$n的脖子上套了上去。" NOR;
}
