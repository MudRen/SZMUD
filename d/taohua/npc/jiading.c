...
int do_recognize(string arg)
{
	...
	if ( mapp(skill_status = me->query_skills()) ) {
		...
		for(i=0; i<sizeof(skill_status); i++) {
			if ( skill_status[sname[i]] > level1 ) level1 = skill_status[sname[i]];
			if ( skill_status[sname[i]] > level ) level = skill_status[sname[i]];
		}
		
                if ( me->query( "serverbonus" ) ) level = level*3/4;
                level = level;
                  s_level = (int)level;
		...
		ob->set("gain",300 + level1 ); // add 150, use qmdj, music too
		gain = ob->query("gain");
		if(gain>600) gain = 600;
		if ( exp > 6400000 ) 
		{
		if ( gain > 450 ) gain = 450; // decrease th yapu gain since we wanted to promote th growth for < 6.4m
		}
		...
		if(exp <  100000) //100k
			...
			gain = gain + random(level/5);
                ...
                if ( exp > 6400000 && gain > 700 ) 
                        gain = gain*3/4 + random( gain/4 );
                ob->set("gain_exp", gain*15/10);      //提高50%的报酬
		ob->set("gain_pot", gain/2 +random(gain/5));//提高100%潜能
	...
	ob->add("gain_exp", random(20)+45);//提高50%报酬
	ob->add("gain_pot", random(10)+30);//提高50%报酬
	...
	ob->set("real_message", HIR"僧人脸色一变，狞笑道：好哇，你居然认出了"+RANK_D->query_self_rude(ob)+"来，需容你不得！\n"NOR);
		...
		case 2 :
			...
			ob->set_skill("longzhua-gong", s_level/2);
	...
	ob->set_skill("tianshan-zhang", s_level);
	//去掉基本毒技ob->set_skill("poison", level);
	ob->set_skill("dodge", s_level);
	ob->set_skill("staff", s_level);
	...
	ob->prepare_skill("staff", "tianshan-zhang");
	...
	weapon=new("/d/xingxiu/obj/staff1");
...
int do_bring(string arg)
{
	...
	yapu_pot = ob->query("gain_pot");
        //提高100%上限
	if(yapu_exp > 1000) yapu_exp = 1000+random(40);
        if(yapu_pot > 500) yapu_pot = 450+random(50);

	yapu_exp = yapu_exp*3/4;
	yapu_pot = yapu_pot*3/4;
        yapu_exp = yapu_exp/2;
        yapu_pot = yapu_pot/2;
	...
	destruct(jiading);
		...
		command("ok");
		me->add("th_exp",yapu_exp*3/2);
		me->add("th_pot",yapu_pot*3/2);
	...
	me->set("th_yapu_ok",1);
		...
		//提高累积经验潜能上限十倍
		if(me->query("th_exp")> 100000) {
		me->set("th_exp",100000);
		tell_object(me,"你的桃花经验已经达到100000点的满点，不能再累积了。\n");
		}

		if(me->query("th_pot")>me->query("max_potential")*100) {
		me->set("th_pot",me->query("max_potential")*100);
