...
mixed hit_ob(object me, object victim, int damage_bonus)
{
	...
	if ( !me->query( "serverbonus" ) && !victim->query( "serverbonus" ) )
		bonus = bonus/2;

	me->apply_condition("th_gain", me->query_condition("th_gain")+1);
	me->add( "exp_gain", (int)bonus );
	victim->add( "exp_gain", (int)bonus );
	me->set_temp("partner",victim->name());
	victim->set_temp("partner",me->name());
//	me->add("combat_exp", (int)bonus);
	me->add( "pot_gain", (int)bonus );
	victim->add( "pot_gain", (int)bonus );
	set_heart_beat(1);
	return;
}
