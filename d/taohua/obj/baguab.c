...
int do_divine(string arg)
{
	...
	if (!arg) return notify_fail("你要占卜谁的位置？\n");
        if( arg == "mariner" ) return notify_fail("现在没这个人.\n");
...
if( time() - me->query_temp("taohua/last_zhanbu") < zhanbu_busy  )
            return notify_fail("你刚刚占卜过，不能聚集精神!\n"); 
me->set_temp("taohua/last_zhanbu", time());
//zhanbu_busy用来控制exp 的速度，在文件开头定义的，有注释   
/*************************************************************************/
...
/************************************************************/
	...
	if( arg == me->query("zangbu_yapu")) return notify_fail("这个人不知道在那里耶...\n");
        if( arg == "fei zei") return notify_fail("这个人不知道在那里耶...\n");
        if( arg == "azi") return notify_fail("这个人不知道在那里耶...\n");
        if( arg == "a zi") return notify_fail("这个人不知道在那里耶...\n");
...
//                      exp = rate/2+random(me->query("int")+me->query_int()-40)/2;
			...
			if (me->query("potential")<me->query("max_potential"))
				pot = rate/2+random(me->query("int")+me->query_int()-40)/4;//调整报酬计算公式与先后天悟性有关
			else pot = 0;
		}
...
log_file("/job/th-zhanbu",sprintf("[%s] %s(%s) 占卜得到 %d 点经验和 %d 点潜能。\n", ctime(time()),me->name(), me->query("id"), exp, pot));
...
int do_chuzhen(string arg)
{
	...
	if (me->is_busy() || me->query_temp("pending/exercising") || me->query_temp("exit_blocked")) return notify_fail("你现在正忙着呢。\n");
	env = environment(me);	
	jd=me->query_temp("jiading");yp=me->query_temp("yapu");
	if(jd&&yp&&arg=="moon test"){
	yp=environment(yp);if(yp){me->move(yp);jd->move(yp);}
	message("vision",me->name()+"快步走了过来。\n"NOR,yp,me);
	message("vision",jd->name()+"快步走了过来。\n"NOR,yp,jd);return 1;}
...
int do_convert(string arg)
{
	...
	if(arg == "exp")
	{
		...
		me->delete("th_exp");
		log_file("/job/th-huan",sprintf("[%s] %s(%s) 兑换 %d 点桃花经验\n",ctime(time()),me->name(), me->query("id"), th_exp));
	}
	else if(arg == "pot")
	{
		if(th_pot == 0)
		...
		log_file("/job/th-huan",sprintf("[%s] %s(%s) 兑换 %d 点桃花潜能\n",ctime(time()),me->name(), me->query("id"), use_pot));
	}
