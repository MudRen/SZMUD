...
{
	seteuid(getuid());
	set_name(HIR"镖车"NOR, ({"biao che", "che"}));
	set("long",
		"这是一辆非常结实的镖车，由两匹大骡子拉着，不知装着什么东西。镖车上竖着一面大旗，上书“奉天承运”四个大字。\n");
	set("unit", "辆");
	set_weight(10000);
...
void host_appear()
{
	...
	if( !target )
	{
		...
		else if ( query_temp("biao/dest") == "wu tiande" ) target = new ("/d/chengdu/npc/wutiande.c");
		else if ( query_temp("biao/dest") == "shi lang" ) target = new ("/d/quanzhou/npc/shilang.c");
		else if ( query_temp("biao/dest") == "le kexiu" ) target = new ("/d/city/npc/kexiu.c");
		else if ( query_temp("biao/dest") == "liu haoyi" ) target = new ("/d/chengdu/npc/liuhaoyi.c");
	}
	else
	{
...
void award2(object dest,object owner)
{
	...
	if( userp(owner) && owner->query("biao/dest2") )
	{
		...
		owner->set("biao/bonus",800+random(700) );
	}
}
...
void award(object dest,object owner)
{
	...
	if(userp(owner))
	{
		...
		owner->delete_temp("riding");
		message_vision("$N上前对着$n抱拳道：“这位"+RANK_D->query_respect(dest)+"请了。"+RANK_D->query_self(owner)+"总算完成任务，现将镖货原物奉上。请您清点！”\n",owner,dest);
		message_vision("$N满脸堆笑的说：“这位"+RANK_D->query_respect(owner)+"果然英雄了得。"+RANK_D->query_self(dest)+"多谢了！”\n",dest);
...
int auto_check()
{
  ...
  if( !owner || !owner->query("biao/dest2") )
  {
	...
	if( strsrch(base_name(here), "/d/city") == -1 &&
		strsrch(base_name(here), "/d/chengdu") == -1 &&
		strsrch(base_name(here), "/d/beijing") == -1 &&
		strsrch(base_name(here), "/d/beijing/zijin") == -1 &&
...
void do_leave(object ob)
{
		if( objectp(ob) )
		{
			if( ob->is_busy() || ob->is_fighting())
			{
				...
				if( random(2)==0 )
					message_vision("$N说道：没想到官府的还真有两下子。"+RANK_D->query_self(ob)+"今天暂且饶你一条狗命！\n",ob);
				else
...
int do_attack(string arg)
{
	...
	message_vision(HIR "$N对着$n"+HIR"喝道："+RANK_D->query_rude(target)+HIR"也想劫镖？来人啊，给我将$p拿下！\n\n" NOR, me, target);
...
int do_check()
{
	...
	else if ( obj->query_temp("biao/dest") == "mi zheng" ) write("这是扬州水陆提督李克秀的重镖，由"+obj->query("owner")+"压运，务必交到水陆提督李克秀手里。\n");
	else if ( obj->query_temp("biao/dest") == "liu haoyi" ) write("这是成都世袭子爵刘好弈的重镖，由"+obj->query("owner")+"压运，务必交到世袭子爵刘好弈手里。\n");
