...
void init()
{
	...
	if ( query("con") == 2 ) {
		environment(me)->set("long",
                "石壁上有几百幅用利器刻成的简陋人形，每个人均不相同，举手投足\n"
...
int do_dig()
{
	...
	if ( !me->query_temp("dig") && query("box1") >= 1 && random(10) == 5 ) {
		message_vision("忽然叮的一声，铁锹碰到一件铁器。$N移近火折一看，原来是一只铁盒。\n", me);
		add("box1", -1);
