...
void create()
{
	...
	set_skill("force", 20);
        ...
        set("inquiry", ([
            "name" : "我也不知道我原来叫什么。",
            "红花阁" : "我们红花阁可是长安城里头一份的找乐子去处。",
            "here" : "我们红花阁可是长安城里头一份的找乐子去处。",
...
int do_sell(string arg)
{
    ...
    if (ob->query("hooker"))
       return notify_fail("她已经是我们红花阁的人了，你不能再卖了。\n");
    ...
    message_vision("$N把"+ ob->name()+"卖到了红花阁。\n", this_player());
    ...
    if (ob->query("age")<18)       
        ...
        ob->set("title","红花阁皇后");
    else ob->set("title","红花阁首席");
    if( ob->query_temp("top_girl") ) {
        ob->set("title",HIR"红花阁花魁"NOR);
	ob->delete_temp("top_girl");
    }
    ob->set("nickname",nick);
    command ("chat 我们红花阁新来了「"+nick+ "」"+ob->name()+"，欢迎各位大爷光顾！");
    ob->set("hooker",1); 
    return 1;
}
