...
稳，好象是个会家子。往上远眺，似乎可以看到石檐一角。这里
马上就要到少林了，为了不打扰庙里的清净，你不得不放慢了脚
步。
LONG
	);
...
void init() {
    ...
    if ( me->query("family/family_name") == "少林派" && me->query("family/generation") < 38
         && time()>me->query("mp_job_time")+180 && !present("sengren", environment(me)) ) {
