...
string find_target_room(object traverser, int dist) {
    ...
    if (strsrch(room_path, "/d/beijing/")==0 ||
        ...
        strsrch(room_path, "/d/shenlong/")==0 ||
             strsrch(room_path, "/d/mingjiao/")==0 || 
...
//    }
if ((int)player->query("max_neili") < ((int)player->query_skill("force") * (int)player->query("con") * 2 / 3))
{
    if (random(exp_rate) > 90) {
        player->add("max_neili", 2);
        player->add("eff_jingli", 2);
}
    }
