...
string ask_win()
{
	...
	msg ="说到当今【神州】第一象棋国手，那当然是"+name+"！其成绩是"+chinese_number(all)+"战"+chinese_number(win)+"胜。\n";
...
string ask_lose()
{
	...
	if ( !obj ) {
		msg ="全【神州】居然没有人下象棋输过，到底是个个都是大国手，还是没人会下棋？\n";
		return msg;
	}
	win = obj->query("c_chess/win");
	draw  = obj->query("c_chess/draw");
	...
	name = obj->query("name")+"("+obj->query("id")+")";
	msg ="放眼【神州】，下象棋输得最惨的就数"+name+"了，成绩是"+chinese_number(all)+"战"+chinese_number(lose)+"败。\n";
