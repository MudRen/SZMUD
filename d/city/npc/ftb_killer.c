...
void set_status(mixed player_arg) {
    ...
    if ( exp < player->query("combat_exp")*4/5) exp = player->query("combat_exp" );
...
// }
      if (exp > 6000000) { //有原来的1m开始增加难度改到6m
         exp_rate += 7*exp/6000000;
          skill_rate += 7*exp/6000000;
}
    exp_rate += random(20);
    skill_rate += random(10);

    if ( exp_rate < 70 ) exp_rate = 80;
    if ( skill_rate < 80 ) skill_rate = 85;
    menpai = set_skills(0, exp/100*exp_rate, skill_rate);
    set("long", "他看起来是"+menpai+"的高手。\n");
