...
int do_join(string arg)
{
        object me;
        me = this_player();
        if (!arg || arg != "商业协会") return notify_fail("黄真奇怪的对你说：“你要加入什么？我这里是商业协会。”\n");
        if (me->query("is_vendor")) return notify_fail("黄真奇怪的对你说：“你已经是商人了啊！”\n");
        if (me->query("bad_vendor")) return notify_fail("黄真叹息道：“你过去既然已经脱离了商业协会，现在就不要再想当商人了！”\n");
        tell_object(me,HIY"黄真认真的对你说：“一个人只能有一次机会加入商业协会，你可要好好想想，决定(decide)再告诉我。”\n");
...
int do_unjoin(string arg)
{
        object me;
        me = this_player();
        if (!arg || arg != "商业协会") return notify_fail("黄真奇怪的对你说：“你要脱离什么？我这里是商业协会。”\n");
        if (!me->query("is_vendor")) return notify_fail("黄真奇怪的对你说：“你现在并不是商人了啊！”\n");
...
int do_yao()
{
        ...
        if (!me->query("is_vendor")) return notify_fail("黄真笑着对你说：“你又不是商人，到我这里来要什么？要饭？”\n");
        if (present("shangye zhizhao",me)) return notify_fail("黄真生气的撇了撇嘴：“商业执照不是好好的在你身上？怎么又来问我要？”\n");
        // if (HEIDIAN_D->is_owner(me->query("id"))) return notify_fail("黄真呵呵笑着说：“你现在是大老板，还要什么商业执照呢！”\n");
