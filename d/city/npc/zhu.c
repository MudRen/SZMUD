...
//add by wzfeng  
	...
	&& ob->query("money_id") && ob->value() >= 100) {
		...
		who->add_temp("mark/朱", ob->value() / 2);
		if (who->query("连城诀/朱熹")) 
                who->add_temp("mark/朱", ob->value() / 2);
		return 1;
	}
	...
	&& who->query_skill("literate", 1) < 90
        && ob->query("money_id") && ob->value() >= 100) {
                message_vision("朱熹同意指点$N一些读书写字的问题。\n", who);
                who->add_temp("mark/朱", ob->value() / 5);
                if (who->query("连城诀/朱熹")) 
                who->add_temp("mark/朱", ob->value() / 8);
                return 1;
        }
	...
	&& who->query_skill("literate", 1) < 120
        && ob->query("money_id") && ob->value() >= 100) {
                message_vision("朱熹同意指点$N一些读书写字的问题。\n", who);
                who->add_temp("mark/朱", ob->value() / 16);
                if (who->query("连城诀/朱熹"))
                who->add_temp("mark/朱", ob->value() / 16);
                return 1;
        }
	if (who->query_skill("literate", 1) >= 120
        && ob->query("money_id") && ob->value() >= 100) {
                message_vision("朱熹同意指点$N一些读书写字的问题。\n", who);
                who->add_temp("mark/朱", ob->value() / 32);
                if (who->query("连城诀/朱熹"))
                who->add_temp("mark/朱", ob->value() / 32);
                return 1;
        }
	return 0;
	//         学费降低为原来的1/50  (by Yujie)
}
