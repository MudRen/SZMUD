...
string ask_mudage();
string ask_lit();
string ask_divorce();//"离婚，就是新生活的开始了。" 
...
string ask_beauty()
{
	...
	name1 = obj1->query("name")+"("+obj1->query("id")+")";
	name2 = obj2->query("name")+"("+obj2->query("id")+")";
	msg ="嘻嘻，现在【神州】里数"+name1+"最漂亮，"+name2+"最英俊。\n";
	return msg;
}
...
string ask_age()
{
	...
	name = obj->query("name")+"("+obj->query("id")+")";
	msg ="嘻嘻，除了大米和神仙外，现在【神州】里数"+name+"年纪最大。\n";
...
string ask_divorce()//"离婚，就是新生活的开始了。" 
{
	...
	if (ob->query("gender") == "男性") xb = "她";
	else xb = "他";
	
        if (!ob->is_married()) 
        	return "离婚，就是新生活的开始了。";

      	add_action("do_lihun","nod2");
       	add_action("do_lihun","nod");
       	return HIG"你真的不再爱" + xb + "了吗？\n也许你们离了以后不会再复合了。\n如果你已经决定了离婚就用nod 或者 nod2来回答！\n"NOR;
}
int do_lihun()
{
        object ob = this_player();
        
        message_vision(CYN"$N默默地点了点头。\n"NOR, ob);
        tell_object(ob,"既然你这样决定了，就成全了你吧。\n");
	log_file("/test/marry",sprintf("[%s] %s(%s) 决定和 %s(%s) 离婚。\n", ctime(time()),this_player()->name(), this_player()->query("id"),this_player()->query("spouse/name"), this_player()->query("spouse/id") ));
        MARRY_D->break_marriage(ob);
        return 1;
}
