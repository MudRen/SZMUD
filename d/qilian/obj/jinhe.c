...
// MODIFY BY SCATTER
// ANTI-ROBOT
//【巫师】布武(Buwu)：scatter，摧毁锦盒的时候提示：一时忙乱中，你的锦盒不翼而飞了。
...
int do_lian(string arg)
{
    ...
    if(stringp(player->query("robot_answer")))
        return notify_fail("请你回答问题！！\n");

	write("\n\n"HIW"在你正准备修炼的时候，蜘蛛举了举前足，似乎想问你一个问题... \n\n\n\n问题产生中..."NOR"\n\n\n");
...
void no_answer(object user)
{
        user->delete("be_robot");
		user->delete("robot_answer");
        tell_object(user, "一时忙乱中，你的锦盒不翼而飞了。\n");
