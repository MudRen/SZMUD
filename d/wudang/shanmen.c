...
void init()
{
        ...
        add_action("do_exercise", "sleep");
                add_action("do_fail", "hit");
                add_action("do_fail", "throw");         
                add_action("do_fail", "xiulian");
                add_action("do_fail", "steal");
                add_action("do_fail", "tuxi");
}
...
int valid_leave(object me, string dir)
{
        ...
        object wp, *inv;
        int i, j=0;
        int allow = me->query_temp( "wudang/allowed" );
        ...
        if ( dir == "southup" && (me->query("shen") < -99)
                && !objectp(present("incense", me)) && !allow)
        {
    ...
    if ( objectp( present( "shou wei", environment( me ) ) ) )
       return notify_fail("山门守卫对着你喝道：站住！武当乃清静之地，你若不是来敬香的就请回吧。\n");
            
        if (j) return notify_fail("\n");    
        }
        
        if ( allow )
            me->delete_temp( "wudang/allowed" );
        return    
