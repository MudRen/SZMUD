...
void setup_skill(object me,object obj)
{
    ...
    */
    obj->set( "combat_exp", obj->query( "combat_exp" )*2/3 );
	level = pow( obj->query("combat_exp")*10.,0.33333 );
	obj->set( "no_teach_bt_npc", 1 );
	...
	else if( obj->query("combat_exp") < 300000)
	{
       ...
       else if( obj->query("combat_exp") < 6400000)
        {
	...
	{
	    if ( names[i] != "literate" )
	        obj->set_skill(names[i], level*2/3);
	    else
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	...
	if( (owner=present(me->query("owner"),here)) )
	{
        ...
        if(owner->query("combat_exp")<6400000)
        {
            exp = 7 + random(6 );
            pot = 2 + random( 4 );
            lvl = 5 + random( 4 );
