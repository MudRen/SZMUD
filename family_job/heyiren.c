...
void finish_time_up()
{
	...
	strID = query_temp("pkid");
	me = find_player(strID);

	//me->add("menpai/gxd", -10);
	//tell_object(me,HIR"你的时间已到但是未完成任务，你对门派的忠诚度下降了！\n");
	//log_file("/job/fail_job", ob->query("name") + "(" + ob->query("id") + ") 的忠诚度被扣了二十点  "+ctime(time())+"\n");
...
{ 
    ...
    if( objectp(ob = me->query_temp("last_damage_from")) ) 
	{
		...
		if(random(100) > 92)
		{
			ob->add("shenzhou/pts", 1);
			tell_object(ob,HIC"恭喜你获得神州贡献度\n");
			log_file("/job/fj_gxd", ob->query("name") + "(" + ob->query("id") + ") 获得了一点神州贡献度  "+ctime(time())+"\n");
		}
		...
		remove_call_out("finish_time_up");
