...
string reward_func(object player, string job, int exp_rate, int pot_rate)
{
    ...
    if ( exp_reward < 0 )
        exp_reward = 0 - exp_reward;
    if ( pot_reward < 0 )
        pot_reward = 0 - pot_reward;
    ...
    if ( job == "wudang_volunteer" )
    {
        log_file("job/wudang",player->query("name") +" (" + player->query("id") + ") 武当守门获得 " + 
            exp_reward + " 点经验和 "+ pot_reward + " 点浅能 "+ctime(time())+"\n");
