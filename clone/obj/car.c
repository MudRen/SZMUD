...
void create()
{
	set_name("大车", ({ "da che", "dache", "carriage"}) );
	set_weight(10000);
...
//增加一个限制，玩家不能带着别的东西坐车。  Piao 修改于2002-03-26        
        for (i=0; i<sizeof(inv); i++) 
        { 
	        if ( userp(inv[i]) ) 
        	return notify_fail("车夫盯着你看了一眼，说道：这车只能载一个人，你把人放下再走吧。\n"); 
	...
	if( me->query_condition("hz_job") ) {
        ...
        if (((string)me->query("job/description")=="守卫山门")&& (objectp(present("zhike daozhang", environment(me)))))
        {
                write ("你的守门工作还没完成，不能离开。\n");
                return 1;
        }
	...
	if( arg == "wudang" || arg == "武当" ) {
		...
		destin = "/d/gumu/gumu";
		destch = "古墓";
		destmg = "这里就是杨过与小龙女双宿双栖的古墓了，要拜师的话，你去向孙婆婆打听吧。                ";
	} 
        else if( arg == "wulin" || arg == "武林" ) {
		destin = "/d/wulin/shanmen";
		destch = "武林同盟";
		destmg = "这里就是著名的武林同盟了，听说这里的主人最近大发善心，允许初出道的小人物随意进入这里，与众高手一起商讨武林大师，小的也因此多了条生意路子。";
	}
        else if( arg == "beijing" || arg == "北京" ) {
                destin = "/d/beijing/kedian";
                destch = "北京";
                destmg = "这是北京客店。尤其京城里新鲜事多着呢，您慢慢逛吧。";
}
        else if( arg == "kunlun" || arg == "昆仑山" ) {
                destin = "/d/kunlun/shanmen";
                destch = "昆仑";
                destmg = "这就是昆仑山了，这里可不比别地方，天气凉着呢。您小心点逛吧。";
}
        else if( arg == "changan" || arg == "长安" ) {
                destin = "/d/changan/kezhan";
                destch = "长安";
                destmg = "长安可是大城市，您去转转，我在这儿等您一会儿。";
}
        else if( arg == "kaifeng" || arg == "开封" ) {
                destin = "/d/kaifeng/kezhan";
                destch = "开封";
                destmg = "这里是开封，听说这里有位青天大老爷，要是您有什么冤案告诉他就行了。";
}
