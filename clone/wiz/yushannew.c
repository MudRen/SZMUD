...
int do_makelist(string arg)
{
	...
	ob->move(me);
	message_vision(HIY"$N把扇子摇了摇，摇出了一张【神州】风云榜。\n"NOR, me);
	return 1;
}
...
int do_sort(string arg)
{
	...
	if(sorttime > 2) return notify_fail("还 sort, 再 sort 【神州】要变 lag 了。\n");
	else {
		sorttime += 1;
    ...
    if (arg=="exp")
	{
        ...
        else if (arg=="per")
	  {
	    ...
	    for (i=max-1;i>=0;i--)
	      {
		obj1 = ob_list[i];
                name = obj1->query("name")+"("+obj1->query("id")+")"+" per: "+obj1->query("potential");
		printf("No. %d: %s\n",i+1,name);
	      }
	    return 1;
	  }
