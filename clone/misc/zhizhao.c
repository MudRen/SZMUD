...
int do_bantan()
{
        ...
        if (!me->query("is_vendor")) return notify_fail("你又不是商人，瞎折腾什么啊！\n");
        if (!present("shangye zhizhao",me)) return notify_fail("你的商业执照不在身上，难道想无证搬摊？\n");
        if (me->query_temp("on_bantan")) return notify_fail("你已经占了一个摊位，还想怎的？\n");
...
int do_shoutan()
{
        ...
        if (!me->query_temp("on_bantan")) return notify_fail("你并没有摆摊，哪里来的摊子给你收啊？\n");
...
int do_stock(string arg)
{
        ...
        if (!me->query("is_vendor")) return notify_fail("你又不是商人，瞎折腾什么啊！\n");
        if (!present("shangye zhizhao",me)) return notify_fail("你的商业执照不在身上，请退线后重新连线并申请一个新的。\n");
        if (!me->query_temp("on_bantan")) return notify_fail("你必须首先摆一个摊位(baitan)才能放货物！\n");
        ...
        if (goods->query("money_id")) return notify_fail("你疯了？钱也拿来出售？\n");
        if (goods->is_item_make()) return notify_fail("你也太狠了吧？定做的东西用来坑人家的钱？\n");
        if (sizeof(me->query("vendor_goods")) >= 10) return notify_fail("你一次只能摆上十种货物，请首先收起几种货物再摆上这种货物。\n");
...
int do_unstock(string arg)
{
        ...
        if (!me->query("is_vendor")) return notify_fail("你又不是商人，瞎折腾什么啊！\n");
        if (!present("shangye zhizhao",me)) return notify_fail("你的商业执照不在身上，请退线后重新连线并申请一个新的。\n");
        if (!me->query_temp("on_bantan")) return notify_fail("你连摊子都没有搬，还想收起货物？\n");
...
int do_checksellerrating()
{
	...
	me = this_player();
	tell_object(me, "你目前的商人评价是："+me->query("n_Seller_Rating")+"点。\n");
	return 1;
}
