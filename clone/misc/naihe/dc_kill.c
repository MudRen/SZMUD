...
int do_start(string arg)
{
	...
	if(t_play==1)
	    log_file("duchang/KILL_GAME", sprintf("\n虚拟战斗游戏更新了，由%s(%s)于%s开始了第一次游戏。\n",me->query("name"), getuid(me), ctime(time())));
	...
	obname=tempa+tempb;
	message_vision(HIW"$N拿出一两黄金塞进机器里，游戏开始了！！！\n"NOR,this_player());
...
// log 文件记录。
	        log_file("duchang/KILL_GAME", sprintf("%s(%s)于%s在虚拟战斗第 %d 次游戏中获 %d 银.\n",
                me->query("name"), getuid(me), ctime(time()),t_play, jj_all));
