...
// 记录到 log 文件里。
	        log_file("duchang/BOSS", sprintf("%s(%s)于%s玩骰子游戏赢了%s两黄金。\n",
                me->query("name"), getuid(me), ctime(time()), chinese_number(jjout)));
...
// 记录到 log 文件里。
	        log_file("duchang/BOSS", sprintf("%s(%s)于%s玩斗牛游戏赢了%s两黄金。\n",
                me->query("name"), getuid(me), ctime(time()), chinese_number(jjout)));
...
//		me->move("/d/city/dc_dating");
	...
	if(mea==oba)
	{
                if(meb>obb) winner="player";
                if(meb<obb) winner="npc";
                if(meb==obb) winner="none";
	}
...
// 记录到 log 文件里。
	        log_file("duchang/BOSS", sprintf("%s(%s)于%s玩一张定输赢游戏赢了%s两黄金。\n",
                me->query("name"), getuid(me), ctime(time()), chinese_number(jjout)));
