...
#include <ansi.h>
#include <armor.h>
#define CLUB_NAME HIW"天"NOR HIG"仙"NOR HIC"一"NOR HIM"族"NOR
#define CLUB_ID "tianxian"
#define CLUB_CHANNEL "tianxian"
#define CLUB_MARK "tianxian ling"
inherit ITEM;
...
void create()
{
	set_name(HIR+CLUB_NAME+HIR+" 令"NOR,({CLUB_MARK,"ling"}));
	set_weight(60);
	...
	else
	{
		...
		set("unit","块");
	set("long",@LONG
这是一块金色的令牌, 用来证明自己是某帮派的一份子。
你可以用(help 本帮派的代号)来查看有什么指令。若想了解本帮派的
的历史可以用(about 帮派 id)来取得进一步的资料。
...
string show_level(object user)
{
 ...
 mapping info;
 string str;
 if(wizardp(user)) return "【荣誉会员】"+CLUB_NAME+"特级顾问  ";
 ...
 switch(rank)
 {
	case 6: str+=CLUB_NAME"龙头老大  "; break;
	case 5: str+=CLUB_NAME+"副会主    "; break;
	case 4: str+=CLUB_NAME+"长老      "; break;
	case 3: str+=CLUB_NAME+"超级护法  "; break;
	case 2: str+=info["info1"]+"分堂主    "; break;
	default: 
		if(info["info1"]!="无")
		{
			str+=info["info1"]+"堂员      "; break;
		} else str+=CLUB_NAME+"一般会众  ";
 }
 return str;
}
...
int do_accept(string arg)
{
	...
	return notify_fail("指令错误:jieshou id title,(title 可以加如颜色!\n");
	else
	{
		ob = present(arg, environment(me));
	...
	else
	{
		...
		if(sizeof(ob_info))
		{
			...
			else
			{
				...
				tell_object(ob,"你被免除了"+sub+"分堂主职务。\n");
				mark->move(ob);
			}
			ob->save();
			return 1;
...
int do_handover(string arg)
{
	...
	else
	{
		...
		if(sizeof(ob_info))
		{
			...
			message( "channel:CLUB_CHANNEL",CYN"【"+CLUB_NAME+"】"+me->name(1)+"("+capitalize(me->query("id"))+")将"+CLUB_NAME+"指挥权移交给"+ob->name(1)+"("+capitalize(ob->query("id"))+")。\n"NOR, users() );
			ob->set("title",HIY+CLUB_NAME+"  "+HIR+"会主"NOR);
			ob->save();
			me->set("title",HIY+CLUB_NAME+"  "+HIR+"前任会主"NOR);
			me->save();
			return 1;
		}
...
int do_leaveclub(string arg)
{
	...
	message( "channel:CLUB_CHANNEL",CYN"【"+CLUB_NAME+"】"+me->name(1)+"("+capitalize(me->query("id"))+")主动退出了"+CLUB_NAME+"。\n"NOR, users() );
...
int do_help(string arg)
{
 if(arg==CLUB_CHANNEL)
 {
  write(@HELP
       ...
       └───────────────┴─────────┴──────┘
	 ◎ 使用 jieshou 指令必须对方先用 join 指令申请才能生效。
	 ◎ 本(帮、会、派)目前有三个分堂: 【天堂】、【地狱】、【人间】。
...
int do_club(string arg)
{
	...
	if (!me->query("in_club") ) return notify_fail("你不是任何帮派成员!\n");
...
int do_about(string arg)
{
 if(arg==CLUB_ID)
 {
  write(HIR"●"+CLUB_NAME+HIG+"信物－"+HIC+"
