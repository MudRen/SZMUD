...
void create() {
        set_name("上诉留言板", ({ "board" }) );
        set("location", "/d/kaifeng/kaifengfu");
        set("board_id", "kff_b");
        set("long", "玩家对巫师提意见专用的留言板，不许灌，违者严惩不怠。\n" );
        set("capacity", 1000);
        set("no_get", 1);
        move("/d/kaifeng/kaifengfu");
        setup();
        restore();
}
...
int do_post(string arg) {
	...
	if (poster->query_temp("kff/my") != "can")
                return notify_fail("你不能随意留言。\n");   
        if (wizardp(poster))
...
int do_read(string arg) {
        ...
        if ( ((int)SECURITY_D->cmp_wiz_level(this_player(), "(arch)") < 0 ) && strcmp(the_player->query("id"), notes[num]["threadID"]))
