...
void create()
{
    ...
    set("inquiry", ([
        ...
        "秘籍" : "现在心法由谭处端师兄保管，你去找他要吧！",
        "miji" : "现在心法由谭处端师兄保管，你去找他要吧！",
        "任务" : (: ask_job :),
        "job" : (: ask_job :),
...
string ask_job()
{
      ...
      if ( ob->query("family/family_name") != "全真教" ){
 		...
 		do_caiyao();
 		return "有劳这位"+RANK_D->query_respect(ob)+"了，一路上多加小心。\n";
      	}
...
/*      if( ob->query("combat_exp") > 100000 && !(random(3)==1) && ob->query_skill("quanzhen-jian",1) > 60 )
      {
          ...
          log_file("job/quanzhen",ob->query("name")+" (" + ob->query("id") + ") 领取练阵任务  "+ctime(time())+"\n");
...
/*      me->add_temp("job_pos", -1);
      ...
      log_file("job/quanzhen",ob->query("name")+" (" + ob->query("id") + ") 领取挖药任务  "+ctime(time())+"\n");
      return "早去早回，路上要小心一些！\n";
*///delete by lisser
	    ...
	    && ob->query_skill("quanzhen-jian",1) > 60 )
	{
		...
		add_action("do_caiyao", "采药");
		add_action("do_lianzhen", "lianzhen");
		add_action("do_lianzhen", "练阵");
		return "现在有练阵(lianzhen)和采药(caiyao)两种工作，你想做哪个工作？\n";

	}
	add_action("do_caiyao", "做");
	add_action("do_caiyao", "zuo");
	return "现在只有采药(caiyao)一种工作，你如果做的话就打“做”或者“zuo”？\n";
}
...
int do_inquiry()
{
    ...
    for(i = 0; i < sizeof(times); i++) {
        ...
        if( !(ob2 = find_player(job[times[i]])) )
        {
         ...
         {
             tell_object(ob, "丘处机告诉你：不过这家伙      object ob2;不知在哪儿.\n");
             continue;
          }
         if( dest->query("no_fight") )
         {
...
int do_caiyao()
{
	...
	{
		command("hmm "+me->query("id"));
		command("say 你不是领任务了吗？为什么还不快去！");
		return 1;
	}
	if ( time() < me->query( "mp_job_time" )+180 )
	    return notify_fail( "我现在没什么事，你等一会再来吧。\n" );
	...
	me->set("qz/jobpot",0);
//	command("give yao chu to "+ob->query("id"));
//	log_file("job/quanzhen",me->query("name")+" (" + me->query("id") + ") 领取挖药任务  "+ctime(time())+"\n");
	command("say 早去早回，路上要小心一些！\n");
	return 1;
}
...
//	log_file("job/quanzhen",me->query("name")+" (" + me->query("id") + ") 领取练阵任务  "+ctime(time())+"\n");
	command("say 你去大校场赵志敬那儿好好练习一下「天罡北斗阵」!\n");
	return 1;
}
