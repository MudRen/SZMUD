	...
	switch(random(5)) {
		case 0:
			if( a * b < 200 ) {
				...
				answer = (a * b) % 10;
				answer = answer * d;
				oper = chinese_number(a) + "乘" + chinese_number(b)+ "的个位数然后再乘"+chinese_number(d)+"等于多少？";
			}
			break;
		case 1:
			answer = a % b;
...
//                if( ob->query("family/family_name") == "丐帮" ) bonus = bonus - 150; // 丐帮 ppl can send letter to get extra bonus
		...
		add_temp("xbiao", -1);
                log_file("/test/XBiaoJob", ctime(time()) + ob->query("name") +"(" + ob->query("id") + ")在神鹰镖局保镖获得" + addexp + "经验和"+ (ob->query("potential")-old_pot) + "潜能\n" );
	}
...
void assign_xbiaotou(object me, object partner)
{
  ...
  ob->set_leader(me);
  message_vision("\n$N快步走了过来!\n", ob);
  message_vision("\n$N对$n抱拳道：“"+RANK_D->query_respect(me)+"，总镖头派我跟着您。俺是粗人，不过对总镖头是忠心耿耿，从今以后，您指东俺绝不往西。“\n\n", ob,me);
}
