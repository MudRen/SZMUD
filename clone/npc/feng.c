...
int check_betrayal(object ob)
{
	...
	if ( mapp(std_status = me->query("students")) ) {
		...
		for(i=0; i<sizeof(std_status); i++) {
			if ( ob->query("family/family_name") != "华山派" && ob->query("id")==stdname[i]
...
int check_student(object ob)
{
	...
	std_id = ob->query("id");
        if ( me->query("shen-player/"+(string)ob->query("id")) <= 0)
                me->set("shen-player/"+(string)ob->query("id"), ob->query("shen"));
	...
	if ( death_time != me->query("students/"+std_id) ){
        ...
        if ( ob->query("shen") < me->query("shen-player/"+(string)ob->query("id")) / 10 ) {
...
int improve_sword(object ob)
{
	...
	if ( my_sword < 119 ) {
		add_level = my_sword + 20 + random(ob->query("int"));
	}else if ( my_sword < 149 ) {
		add_level = my_sword + 10 + random(ob->query("int"));
	}else if ( my_sword < 199 ) {
		add_level = my_sword + 5 + random(ob->query("int"));
	}else {
        ...
        if (add_level >= 1) tell_object( ob, "你的基本剑法增加了"+chinese_number((add_level-my_sword))+"级！\n");
...
void attempt_apprentice(object ob)
{
	...
	if( objectp(ob1 = present("mengzhu", room)) )
        ...
        if ( me->query("betrayer") > 0 || (ob->query("family/family_name") && ob->query("family/family_name") != "华山派" )) {
		if (ob->query("family/family_name") && me->query("betrayer") == 0)
                	command("say "+RANK_D->query_respect(ob)+"即是"+ob->query("family/family_name")+"的弟子，又为何要投入华山门下？");
                command("say 我华山派是名门正派，不收不忠不义之徒！");
		return;
	}
...
/*	if ( (int)ob->query_skill("taiji-jian") > 0 || (int)ob->query_skill("damo-jian") > 0
	...
	log_file("/bai-feng",sprintf("[%s] %s(%s) %s拜华山派风清扬为师。\n", ctime(time()),ob->name(), ob->query("id"), (ob->query("family/family_name") != "华山派")?"从"+ob->query("family/family_name")+"叛师，":""));
