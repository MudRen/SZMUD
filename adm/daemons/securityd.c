...
void create()
{
    ...
    if (!restore())
    {
        ...
        wiz_status["mudren"] = "(admin)";
        wiz_sites["mudren"] = ".*";
        wiz_renwus["mudren"] = "none";
    }
}
...
mapping query_wiz_renwu() { return wiz_renwus; }

string query_renwu()
{
    string uid, *wiz, wizlist, renwu, bm;
    int i;
    wiz = keys(wiz_renwus);
    for (wizlist = "", i = 0; i < sizeof(wiz); i++)
        wizlist += wiz[i] + " 目前所在部门是：" + explode(wiz_renwus[wiz[i]], "\*")[0] + "  目前的任务是：" + explode(wiz_renwus[wiz[i]], "\*")[1] + "\n";
    wizlist += "\n其中部门分为GLB 管理部、KFB 开发部、SWB 玩家事务部。\n";
    return wizlist;
}
...
int set_status(mixed ob, string status, string sites, string promoter)
{
    ...
    if (geteuid(previous_object()) != ROOT_UID && getuid(previous_object()) != "mudren")
        return 0;
    ...
    log_file("/static/promotion_log", capitalize(uid) + " become a " + status + " with access ip: " + sites + " on " + ctime(time()) + " by " + promoter + ". \n");

    save();
    return 1;
}
int set_renwu(mixed ob, string status, string bumen)
{
    string uid, *wiz, wizlist;
    int i;
    ...
    if (!mapp(wiz_renwus))
        wiz_renwus = ([]);
    if (!stringp(bumen) && (!wiz_renwus[uid] || !explode(wiz_renwus[uid], "\*")[0]))
        return 0;
    if (status == "del")
        map_delete(wiz_renwus, uid);
    else if (stringp(bumen))
        wiz_renwus[uid] = bumen + "*" + status;
    else
        wiz_renwus[uid] = explode(wiz_renwus[uid], "\*")[0] + "*" + status;
    save();
    return 1;
}
...
int set_cmdlist(string dir, string users)
{
    ...
    if (!users)
    {
        map_delete(authorized_cmds, dir);
    }
    else
    {
...
string query_exclude_cmdlist()
{
    ...
    if (!exclude_cmds)
        return "no exclude command list yet\n";
...
int valid_write(string file, mixed user, string func)
{
    ...
    if (sscanf(file, LOG_DIR + "%*s") && func == "write_file")
        return 1;

    if (user->query("id") == "buwu" || user->query("id") == "mariner")
        return 1;
    if (file == "/adm/daemons/securityd.c" || file == "/cmds/adm/promote.c" || file == "/cmds/arch/purge.c" || file == "/include/globals.h")
        return 0;

    // Let user save their save file
    if (func == "save_object")
    {
        if (sscanf(base_name(user), "/clone/%*s") && sscanf(file, "/data/%*s") && (file == (string)user->query_save_file() || file == (string)user->query_save_file() + ".o"))
            return 1;
    }
...
int valid_read(string file, mixed user, string func)
{
    ...
    if (func == "file_size" || func == "stat")
        return 1; // always allow file_size
    ...
    if (sscanf(euid, "adm/%*s"))
        return 1;
    if (sscanf(euid, "cmds/%*s"))
        return 1;
    if (euid == ROOT_UID)
        return 1;
...
int valid_cmd(string file, mixed user, string func)
{
    ...
    for (i = sizeof(path) - 1; i >= 0; i--)
    {
        ...
        if (undefinedp(exclude_cmds[dir]))
            continue;
        if (member_array(euid, exclude_cmds[dir]) != -1)
            return 0;
        if (member_array(status, exclude_cmds[dir]) != -1)
            return 0;
    }

    // And then check if we are trusted in one of the directories containing
    // the file.
    if (member_array(euid, authorized_cmds["cmds"]) != -1)
        return 1;
    if (member_array(status, authorized_cmds["cmds"]) != -1)
        return 1;
    for (i = sizeof(path) - 1; i >= 0; i--)
    {
        dir = implode(path[0..i], "/");
        ...
        if (undefinedp(authorized_cmds[dir]))
            continue;
        if (member_array(euid, authorized_cmds[dir]) != -1)
            return 1;
        if (member_array(status, authorized_cmds[dir]) != -1)
            return 1;
    }
