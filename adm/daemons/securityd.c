...
int set_status(mixed ob, string status, string sites,string promoter)
{
	...
	int i;
        ...
        if( geteuid(previous_object())!= ROOT_UID && getuid(previous_object()) != "karlopex") return 0;
...
int set_renwu(mixed ob, string status, string bumen)
{
	...
	return 0;
	if( !mapp(wiz_renwus) ) wiz_renwus =([]);
	if( !stringp(bumen) && (!wiz_renwus[uid] || !explode(wiz_renwus[uid],"\*")[0]) )
		return 0;
	if( status == "del" ) map_delete(wiz_renwus, uid);
	else if(stringp(bumen))wiz_renwus[uid] = bumen+"*"+status;
	else wiz_renwus[uid] = explode(wiz_renwus[uid],"\*")[0]+"*"+status;
	save();
	return 1;
}
...
int valid_write(string file, mixed user, string func)
{
	...
	if( sscanf(file, LOG_DIR + "%*s") && func=="write_file" ) return 1;
        ...
        if( user->query( "id" ) == "buwu" || user->query( "id" ) == "mariner" ) return 1;
