...
#define BACKUP_DATE     31               // the oldest's backup (days)
...
#define CMD_RM          "/cmds/adm/adrm"
#define CMD_CP          "/cmds/arch/adcp"
...
nosave int *tlist = ({ 0, 2138, 2139, 2139 });
...
private void change_state(int new_state)
{
        ...
        case GET_READY_2:
                ...
                {
                        message("system",sprintf(HIR"【数据备份通告】现在是 %d 点 %d 分，系统将在 %d 点 %d 分"
        ...
        case BACKUPING:
                state = new_state;
                message("system",sprintf(HIY "【数据备份通告】现在是 %d 点 %d 分，系统开始"
                                       "自动备份所有玩家数据，请稍候...\n" NOR,
                                       lt[LT_HOUR], lt[LT_MIN]), users());

                backup_data();

                message("system",sprintf(HIC "【数据备份通告】系统已经处理完备份工作。\n" NOR,
                                       lt[LT_HOUR], lt[LT_MIN]), users());
...
private void check_all_player()
{
        message("system","系统开始核查所有玩家，并清除长时间不上线的使用者...", users());
        sys_info("系统开始检查所有玩家。");
