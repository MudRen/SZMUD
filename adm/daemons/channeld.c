...
mapping channels = ([
        ...
        "bangpai":([     "msg_speak": MAG "【帮派】%s：%s\n" NOR,
                        "msg_emote": MAG "【帮派】%s" NOR,
                        "bangpai_only": 1
                        ]),
...
{
        object *ob;
        string *tuned_ch, who, menpai, mps, e_c, arg2;
        // for debug
        int wizmp, color, ce;
            int pts = 0, chatpts = 0;
        ...
        if( verb[sizeof(verb)-1] == '*' ) {
                emote = 1;
		if(wizardp(me) && sscanf(arg, "%s+%s", arg2, e_c) == 2){
			ce = 1;
			arg = arg2;
		}
                verb = verb[0..<2];
	...
	if (wizardp(me) && member_array(verb[sizeof(verb)-2..sizeof(verb)], mp1) != -1) {
        ...
        if ( wizardp(me) && !emote ) arg = setcolor(arg);

        if( userp(me) && !wizardp(me) && verb == "rumor" )
                if(me->query("jingli") > 50) me->add("jingli", -50);
                else    return notify_fail("你已经没力气散播谣言了！\n");

        if( channels[verb]["chat"] ){
                channels[verb]["msg_emote"] = (wizardp(me) ? HIG : HIC)+"【闲聊】" HIC "%s" NOR;
                channels[verb]["msg_speak"] = (wizardp(me) ? HIG : HIC)+"【闲聊】" HIC "%s：%s\n" NOR; 
        }
        ...
        if (channels[verb]["menpai_only"]) {
                ...
                channels[verb]["msg_speak"] = HIW "【"+(wizardp(me) ? HIG : "")+menpai+ NOR+HIW "】%s：%s\n" NOR;
                channels[verb]["msg_emote"] = HIW "【"+(wizardp(me) ? HIG : "")+menpai+ NOR+HIW "】%s" NOR;
        }
		...
		if(channels[verb]["bangpai_only"] && !me->query("club")){
			...
			menpai = me->query("club");
                ...
                channels[verb]["msg_speak"] = MAG "【"+(wizardp(me) ? HIG : "")+menpai+ NOR+MAG "】%s：%s\n" NOR;
                channels[verb]["msg_emote"] = MAG "【"+(wizardp(me) ? HIG : "")+menpai+ NOR+MAG "】%s" NOR;
        }        
        // add points for chat
        ...
        ob = filter_array( users(), "filter_listener", this_object(), channels[verb], menpai, wizmp);
	if ( ce && e_c ) arg = setcolor(e_c+arg,1);
...
int filter_listener(object ppl, mapping ch, string menpaix, int wizmp)
{
        ...
        if( ch["bangpai_only"] ){
		if ( wizmp ) return wizardp(ppl) || ppl->query("club");
        	else return wizardp(ppl) || ( menpaix == ppl->query("club"));
        }
        return 1;
}
