...
// april added suxin-jue change gumu's jing and yunu-xinjing to qi,08/18/2001
// april added kunlun 2001.12.7
...
//	减少max_jing影响,增加max_qi对max_potential的作用
//		my["max_potential"] = 100 + to_int(sqrt(to_float(my["combat_exp"]))) / 10 + (my["max_jing"]-100)/40
//				+(my["max_qi"]-100)/100;
	...
	if( userp(ob) || undefinedp(my["max_jing"]) || (!userp(ob) && !ob->query("special_npc")) ) {
		...
		&& (jing_age =(int)ob->query_skill("suxin-jue", 1)) > 39 )
		{
			jing_age = jing_age/2;
			if (my["age"] <= 30) jing_age -= my["age"];
			else jing_age -= 30;
			...
			if ( jing_age > 0 )
				my["max_jing"] += jing_age * (skill1/12);
		}
		...
		if ( mapp(my["family"]) && my["family"]["family_name"] == "桃花岛"
			...
			if (xism_age > 0 ) my["max_jing"] += xism_age * (skill1/30);
		}

		// 昆仑琴棋书画：３０岁前补精，３０岁后长精，但效用小
		if ( mapp(my["family"]) && my["family"]["family_name"] == "昆仑派"
			&& (xism_age=(int)ob->query_skill("qinqi-shuhua", 1)) > 39 ){
		
			xism_age = xism_age/2;
			if (my["age"] <= 30) xism_age -= my["age"];
			else xism_age -= 30;

			skill1 = (int)ob->query_skill("xuantian-wuji",1);
	...
	if( userp(ob) || undefinedp(my["max_qi"]) || (!userp(ob) &&!ob->query("special_npc")) ) {
		...
		// 古墓玉女心经；３０岁前补气，３０岁后长气
		if( mapp(my["family"]) && my["family"]["family_name"] == "古墓派"
		&& (qi_age =(int)ob->query_skill("yunu-xinjing", 1)) > 39 )
		{
			qi_age = qi_age/2;
			if (my["age"] <= 30) qi_age -= my["age"];
			else qi_age -= 30;
