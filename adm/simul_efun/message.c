/* º®±ù(Michen)ÐÞ¸ÄÓÚ2001ÄêÏÄ Email:mud_wizard@sina.com.cn */
...
void log_file(string file, string text)
{
    ...
    {
        rename( filename, filename + sprintf("%5d",time()) + ".log" ); // the log file may be too long now, rename it, sdong
    }
...
varargs string read_file( string file ,int lines ,int line)
{
    ...
    return efun::read_file( file ,lines ,line);
}
...
void tail(string file)
{
   ...
   if (stringp(file) && file_size(file) >= 0)
    {
        write(efun::read_file(file, file_length(file) - 9));
    }
}
