...
int ask_skill()
{
        ...
        if ( me->query("newjob/jobtitle") == "铁匠" && !me->query_skill("datieshu",1))
                {
		me->set_skill("datieshu",2);
                        message_vision(NOR"白长石指点了$N一些打铁术的知识!\n"NOR,me);
                }
                else
                {
                        message_vision(NOR"白长石对$N说道: 你吓问什么吗？ 我也不会教你!\n"NOR, me);
                }
                return 1;
}
...
int accept_object(object who, object ob)
{
    ...
    if (ob->query("money_id") && ob->value() >= 2000) 
        {
                message_vision("白长石同意指点$N一些铁匠知识的问题。\n", who);
        who->add_temp("tiejiang/bailearn", 4 + random(2));
        return 1;
    }
        if (ob->query("money_id") && ob->value() >= 5000) 
        {
                message_vision("白长石同意指点$N一些铁匠知识的问题。\n", who);
        who->add_temp("tiejiang/bailearn", 10 + random(5));
        return 1;
    }
        if (ob->query("money_id") && ob->value() >= 10000) 
        {
                message_vision("白长石同意指点$N一些铁匠知识的问题。\n", who);
        who->add_temp("tiejiang/bailearn", 20 + random(10));
        return 1;
    }
}
