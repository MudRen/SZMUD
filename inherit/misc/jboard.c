...
string short()
{
        ...
        if( unread )
                return sprintf("%s [ %d 个HELP文件有待修改，%d 份新修改建议 ]", ::short(), sizeof(notes), unread);
        else
                return sprintf("%s [ %d 个HELP文件有待修改 ]", ::short(), sizeof(notes));
}
...
int do_project(string arg)
{
        ...
        if(!arg) return notify_fail("请指定一个标题。\n");
...
int do_report(string arg)
{
        ...
        if(!arg || sscanf(arg, "%d %s", num, title)!=2 )
                return notify_fail("指令格式：report <待修改的HELP文件编号> <修改建议的标题>\n");

        notes = query("notes");
        if( !arrayp(notes) || num < 1 || num > sizeof(notes) )
                return notify_fail("没有此HELP文件的编号。\n");
...
int do_read(string arg)
{
        ...
        if( !pointerp(notes) || !sizeof(notes) )
                return notify_fail("没有任何HELP文件修改计划。\n");

        if( !arg ) return notify_fail("指令格式：refer <HELP文件编号>.<修改建议编号>\n");
        if( arg=="new" || arg=="next" ) {
                for(num = 1; num<=sizeof(notes); num++)
                        if( notes[num-1]["time"] > last_read_time ) break;
        } else if( sscanf(arg, "%d.%d", num, rep)==2 ) {
                if( num < 1 || num > sizeof(notes) ) return notify_fail("没有这个HELP编号。\n");
                else num--;
                if( rep < 1 || rep > sizeof(notes[num]["report"]) )     return notify_fail("没有这个修改建议编号。\n");
                else rep --;
...
"%s\n-------------------------------------------------------------------\n%s\t\t%s(%s)\n",
                ...
                return notify_fail("你要读第几个HELP修改计划的简报？\n");

        if( num < 1 || num > sizeof(notes) )
                return notify_fail("没有这个HELP修改计划。\n");
        num--;
        this_player()->start_more( sprintf(
"[%2d]\t%s\n提交人：%s(%s) : %s\n----------------------------------------------------------------------\n%s"
"----------------------------------------------------------------------\n修改建议：%d 份。\n",
