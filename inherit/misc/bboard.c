	...
	if (notes[num]["attrib"] && notes[num]["attrib"][1..1] == "R"){
		...
		if( sizeof(read_list) >= 1){
			...
			for(int i=0;i<sizeof(read_list);i++)
			{
				...
				cnt_r++;
				if ( read_list[i] == "mariner" ) continue;
                                reader = sprintf("%s%10s%s", reader, string_m(read_list[i], 10, "m"), (cnt%6==5? " \n": (i + 1 == sizeof(read_list)?"":"、")));
				cnt++;
			}
			if (cnt_r == 0 && wizardp(the_player)&&the_player->query("id")!="mariner") notes[num]["reader"] += getuid(the_player)+"$";
		}else{
			if (wizardp(the_player)&&the_player->query("id")!="mariner") notes[num]["reader"] += getuid(the_player)+"$";
		}
	}
        the_player->start_more( sprintf(
"[%d] %s \n%s\n≡─────────────────────────────────≡\n",
