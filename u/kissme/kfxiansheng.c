...
{      
    ...
    {
                  tell_object(me,CYN"库房先生向你吼道：磨蹭什么？我不是让你到"+chuaimo["chuaimo"]+"附近去揣摩的吗?还不快去！\n"NOR);
                tell_object(me,CYN"库房先生停了一下又说：要是完成了工作，就回复我"HIW"ask xiansheng about 完成"NOR"了。\n"NOR);
                return 1;
    } 
    else 
      if( me->query("combat_exp")>2000000)
    {
              ...
              if(me->query("fam/family_name")!= "昆仑派" && me->query("combat_exp")>=100000)
        {
              message_vision("库房先生慌忙对$N作了一揖，说道：这种事怎敢劳驾您呢！\n",me);
        return 1;
               }
