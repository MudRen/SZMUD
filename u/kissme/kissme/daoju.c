...
string *msg_biaoyan = ({
    "$P挥舞着说中的宝剑,表演的正得意的时候,宝剑却脱手而飞,引起人群得一阵慌乱!\n",
    "$P扯着沙哑的嗓子没吼几声就把围观的人都给吓跑了!\n",
    "$P像模像样的站马步,运气,挥掌,一掌下去,手掌被打的鲜血直流。\n",
    "$P走在绳索上，一摇一晃，煞是好看，却不料绳索突然断了，结果摔了一个嘴啃泥！\n",
    });
...
int do_biaoyan(string arg)
{
	...
	if (environment(me)->query_temp("changxi") >= 1)
      ...
      if(rnd==6||rnd==7){
          message_vision("人众里有人大声对$N骂道:剑术这么差,也好意思出来表演,快滚吧!\n",me);
...
int wujian(object me)
{
        ...
        int jlexp=50*(int)environment(me)->query("fanhua")+me->query("int");
          message_vision(HIY"$N将一套剑发施展开来，干净利落,引来了一阵喝采声。\n"NOR,me);
