...
// 最后更新 by buwu : 长时间没有人做工作的 bonus 从 15 min 减为 5 min. 将收益从 600/3600 调整为 600/2400
// 普通收益最低限从 3000 提高到 4000。经验上限从 /1000 提高到 /800
/*
...
斧头帮工作                              
        ...
        400     杀两个刺客
        900     杀三个刺客
        1600    杀四个刺客 外加 2 gold
        如果最近的半个小时有 50  个刺客，收益减25%。
        如果最近的半个小时有 100 个刺客，收益减50%。
...
//      if (time() - last_job > 1200) {  // nobody does job for 20 min
      if (time() - last_job > 300) {  // nobody does job for 15 min
        // increase 600 exp/hour, so 10 hour nobody do job, it's 6K/h
        exp_limit += (time()-last_job)*600/2400;   
        pot_limit += (time()-last_job)*60/2400;   
        if (exp_limit > 10000) exp_limit = 10000;           // cap
        if (pot_limit > 1000) pot_limit = 1000;
    }
    exp_limit -= exp_limit/800;
    if (exp_limit < 4000) exp_limit = 4000;     // exp/hour
...
string tell_job(object player) {
    int start_time, used_time, ftb_job_num, rate, exp_rate, obj_num, kill_num, range, scexp;
    int i, sacrifice;
    ...
    // case 1: over time
    if (used_time >= 600) { // buwu change 20 min to 10 min
    ...
    if (player->query(JOB_NAME+"/kill_num") == 0) {
		...
		//Track the Numeber of Cike Out / 30 min
		add("cike_out", obj_num);
        call_out("do_minus", 1800, obj_num);
		//End of Track
        return "听说有"+num_msg+"个家伙想对本帮不利.
程金斧说道：据说他们已经到了"+region+room_msg+"方圆"+range_msg+"里之内.
程金斧说道：麻烦"+RANK_D->query_respect(player)+"去查一查, 若真是刺客便替本帮主除却了吧.";
    }
	...
	{
		scexp = 1600 + random(80);
	}

	if( query("cike_out") > 100 )
	{
		scexp = scexp / 2;
	}
	else if(query("cike_out") > 50)
	{
		scexp = scexp * 75 / 100;
	}
	
	player->add("combat_exp", scexp);
	player->add("potential", scexp * 4 / 10);
...
//    player->set("ftb_job_num",);
    if (player->query("combat_exp") < 10000) return "小朋友听话，别来瞎掺和，别处玩去。\n";
//    if (player->query("combat_exp") > 11000000) return "这点小事不必麻烦你老人家大驾。\n";  
...
string ask_gonglao()
{
        ...
        if (!who->query("ftb_job_num")<10)
                return RANK_D->query_respect(who)+"才为我帮出了多少力，你问我，我都不好意思说？唯恐江湖人耻笑。";

return RANK_D->query_respect(who)+"为我斧头帮尽心尽力，已经完成任务"+chinese_number(who->query("ftb_job_num"))+"次。";
}
