...
{
	int i, j, k, coin, guaranty, num, index, cheat_times, ob_price, *check_v;
	mixed *ob_value, *v, *v2;
	...
	if(sscanf(arg,"%s for %d %s",str,num,unit) == 3)
	{
		...
		if(ob->query("no_drop")	|| ob->query("no_sell")	|| ob->query("money_id") || ob->is_character() || ob_id == "corpse" || ob_id == "skeleton" || ob_id == "huo ba" || ob_id == "deng long" || ob_id == "la zhu")
		{
	...
	if(sscanf(arg,"%s add %d %s", ob_id,num,unit) == 3)
	{
		...
		*/
		...
		if((int)((num + ob_price)*11/10 > this_player()->query("balance")))
				{
						tell_object(me,"你是想用非法手段牟取暴利吗？（预计价含10%中介费）。\n");
						return 1;
				}
