...
int do_check_menpai_job(string arg)
{
	...
	if( arg ) {
		...
		if(gumu)
		{
			msg+=job_data->query_family_jobdata("古墓派");
			member=job_data->choose_of_player("古墓派","good");
			if(!sizeof(member))
				msg+=HIR"这个门派没有完成任务的完家。\n"NOR;
			else
				if(sizeof(member)!=1)
			...
			member=job_data->choose_of_player("古墓派","bad");
			if(!sizeof(member))
				msg+=HIR"这个门派没有完成任务的完家。\n"NOR;
			else
				if(sizeof(member)!=1)
...
int do_setorg_default(string arg)
{
	...
	obj_temp->add_orgdata("古墓派",strategy_gm,luck_gm,money_gm,luck_slrate,power_gm,assess_gm);
...
int do_check(string arg)
{
	...
	if(!obj_temp->have_menpai("gumu"))
		return notify_fail("没有"+"gumu"+"这个门派。\n");
