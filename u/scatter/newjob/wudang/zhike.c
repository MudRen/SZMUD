...
4       PASS ON DATA STEP 
5        
6         1. YOU NEED TO DEFINE THE FOLLOWING LINE BEFORE INHERIT 
7                        
8                         #define SAVE_NPC "/family_job/savenpc" 
9        
10        TO PASS DATA AND SAVE: 
11        1. TO PASS ON DATA YOU NEED TO CREATE TWO VARIBLES (string pdata, fname;) 
12           THE FORMAT OF IT IS    
13                
14                 fname = "wizname/filename"; 
15                 pdata = "DATA THAT YOU WANT TO BE SAVE"; 
16                  
17                       SAVE_NPC->do_save(fname, pdata); 
18       
19        TO LOAD DATA 
20        2. TO LOAD DATA YOU NEED TO CREATE TWO VARIBLES (string fname, ropen;) 
21           THE FORMAT OF IT IS 
22       
23              ropen = SAVE_NPC->do_load(fname); 
24       
25  
*/ 
...

{
  ...
  if(mppoint > 699 && ob->query("combat_exp") > 500000)
  {
	  command("现在武当工作已不再是第一阶段工作了，你还是去做点别的吧。");
	  return 1;
  }
  ...
  if ( ob->query_temp("wudang/volunteerb") )
  {
          ...
          command("say 你才刚做完你不累呀，我看还是休息一下让别人做吧。");
          return 1;
  }
  //ADD BY SCATTER FOR PREVENT ROBOT VOLUNTEER
  ...
  if (query_temp("stock") > 3)

    {
      ...
      tell_object(ob,CYN"知客道长对你说道：你来晚了，今天的活儿我都分派完了。\n"NOR);
  ...
  if ((int)environment(ob)->query_temp("worker/"+ob->query("id")) 
...

    {
      ...
      tell_object(ob,CYN"知客道长对你说道：这两天山门平安无事，你去帮制香道长砍树吧。\n"NOR);
