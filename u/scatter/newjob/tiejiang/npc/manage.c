...
object me;
::init();
if( interactive(me =this_player())&& !is_fighting()&& (int)me->query_temp("kans"))
{
	add_action("do_da", "da");
...
int ask_question()
{
     ...
     {
     	 tell_object(me,"我不是允许你挖矿了吗?\n");
     	 return 1; //Newbe added
      }
...
int do_da(string arg)
{
        ...
        if( !me->query_temp("kans") )
                return notify_fail("你要答什么呀？\n");
        if( arg == me->query_temp("kans"))
                {

				obj = new("/newjob/tiejiang/obj/lingpai.c");
            ...
            me->delete_temp("kans", 1);
                        me->set_temp("kuangchang/enter", 1);
            return 1;
                }
        
        else if( arg != me->query_temp("kans") )
                {
