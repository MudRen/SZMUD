...
int do_ride(string arg)
{
        ...
        if( me->query_temp("riding") )  return notify_fail("你已经在矿车上！\n");
        if( cart = present("kuang che",environment()) );

        message_vision(HIR"$N"+HIR"神气地跳上了矿车!\n"NOR,me);
        me->set_leader( cart );
...
int do_unride(string arg)
{
        ...
        if( !me->query_temp("riding") )  return notify_fail("你已经不在矿车上！\n");

        message_vision(HIR"$N"+HIR"跳下了矿车！\n"NOR,me);
