...
// ***** FIX THE ANTI-ROBOT BUSY TIME PROBLEM *****
...
//                user->set("robot_ding", user->query("mud_age") + user->query("ding_robot")*600);
				user->add("xingxiu/robottime", 600);
				user->set_temp("xingxiu/failquestion", user->query("xingxiu/robottime"));
...
void fail_question_count() 
{
	...
	if(me->query_temp("xingxiu/failquestion") < 1) 
	{
		me->delete_temp("xingxiu/failquestion"); 
		return;
	}

	me->add_temp("xingxiu/failquestion", -1); 
	remove_call_out("fail_question_count");  
	call_out("fail_question_count", 1); 
	return;
}
...
//        if (me->query("robot_ding") > me->query("mud_age")) 
		if(me->query_temp("xingxiu/failquestion"))
		{
                command("say 哼！上次你骗老仙的事，老仙还没有和你算帐呢！你现在还有脸问这问那！");
                command("say 你先给我回去好好反省自己一下，以后再来！");
				write("你的等待时间还剩 " + me->query_temp("xingxiu/failquestion") + " 秒\n");
                return 1;
        }
        ...
        if (random(4) == 1 && !me->query("passed_ding")) {
                command("say 昨夜老夫受仙人告戒，说你" + me->name(1) + "做事不老实。用些机器来欺骗老夫");
        ...
        if (!present("wa guan", me)) waguan->move(me);
            message_vision("$N顺手从身后架子上取出一个小瓦罐扔给$n。\n",this_object(), me);
        ...
        if(ob->query("id")=="ling pai"){
                if(ob->query("player")!=me->query("id")){
                        ...
                        if (ob->query("menpai")=="huashan" || ob->query("menpai")=="quanzhen")
                                ...
                                //exp triple
                                exp = 280 + random( 60 );
                        } else  exp = 320+random(50);			if (this_player()->query("combat_exp")>6400000)
 exp = exp/2;
                        me->add("combat_exp", exp);
