...
//      } else
        ...
        if( file_size(ob->query_save_file() + __SAVE_EXTENSION__) >= 0 ) {
			...
			if( (time()-(int)ob->query("last_on"))/86400 <= 3 && ob->query("pw-gs") == "yes"){
	                        write_ob(ob,"您的密码已经挂失，请三天后重设你的使用密码！");			
		                destruct(ob);
		                return;
		        }
			if( !ob->query("check_password") ){
				...
				write_ob(ob,HIR"******************************************************************\n"NOR);
	                ...
	                if( (time()-(int)ob->query("last_on"))/86400 >= 3 && ob->query("pw-gs") == "yes"){
				write_ob(ob,"请输入你的保密密码对使用密码进行重置：");
...
private void gs_passwd(string checkpass, object ob)
{
	...
	ob->set("pw-gs","yes");
        log_file("passwd-gs", sprintf("[%s]  %s(%s)对密码进行挂失。\n", ctime(time()), ob->query("name"), ob->query("id")));
	ob->save();
...
private void setnew_password(string pass, object ob)
{
        ...
        input_to("setnewconfirm_password", 1, ob);
}

private void setnewconfirm_password(string pass, object ob)
{
        string my_pass, file, *tmp;
        object user;
        int cnt;
...
private void confirm_password(string pass, object ob)
{
        ...
        if( crypt(pass, old_pass)!=old_pass ) {
				...
				write_ob(ob,HIR"*"NOR"取回你的人物。这样不用再找神仙重发密码，事实上神仙也"HIY"不会再管"NOR"这事"HIR"*\n"NOR);
				write_ob(ob,HIR"*"NOR"了。因此保密密码的保密非常重要，"HIY"请确保没有第二个人知道，并且还需"HIR"*\n"NOR);
				write_ob(ob,HIR"*"HIY"要你在关键时候能想得起来"NOR"。请设置长度至少十位的保密密码。        *\n"NOR);	
				write_ob(ob,HIR"******************************************************************\n"NOR);
        write_ob(ob,"请设定您的保密密码：");
...
private void init_new_player(object user)
{
   	...
   	user->create_human_body();
        CHANNEL_D->do_channel( this_object(), "rumor", sprintf("欢迎新人：%s(%s)来到神州世界。", user->name(),user->query("id")) ); 
