...
//                    npc->set("task_object",1);
	...
	CHANNEL_D->do_channel(this_object(),"chat",HIY"好，江湖恩怨(quest)传书完毕，请各位老少英雄出发吧！\n"NOR);
             this_object()->set("job_start_time",time());
...
int task_give(object me,object who,object ob)
{
	...
	string tmp;
	int timer=0;
		int task_times,job_time;
	if(ob->query("task_owner")==who->query("id")&&userp(me)
	&&task_list[ob->query("task_owner_cname")+"的"+ob->query("name")]!=HIR"(恩怨了结)"NOR)
	{

                //每人完成次数限制
                job_time = this_object()->query("job_start_time");
                if (me->query("taskjob_time") != job_time )
                {       me->set("taskjob_time",job_time);
                        me->set("task_times",1);
                }
                else
                {
                task_times = me->add("task_times",1);
                }
                task_times = me->query("task_times");
                if (task_times > 3)
                {
                message("vision",CYN+who->name()+"说道："+RANK_D->query_respect(me)+"奔波多趟，一定颇为劳累，"
                +RANK_D->query_self(who)+"的事就不忙您操心了。"+NOR,me);
                return 0;
                }       // add bu yujie
