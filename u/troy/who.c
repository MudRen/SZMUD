...
mixed main(object me, string arg, int remote)
{
	...
	j = sizeof(sites);
	 ...
	 str = CYN"『" + INTERMUD_MUD_CNAME + "』"HIW"目前在线的玩家有："NOR"\n";
	 str += "─────────────────────────────────────\n";
	 list = users();
	 total = sizeof(list);
...
/*		 str = sprintf("%s%12s %s\n", str, RANK_D->query_rank(list[j]),
	...
	// Skip those users in login limbo.
		 ...
		 str = sprintf("%s%-15s%s", str,
				opt_id ? capitalize(list[j]->query("id")): (query_idle(list[j]) > 120?"*"+list[j]->name(1):list[j]->name(1)),
				(ppl_cnt%5==4? "\n": "")
			);
		 ppl_cnt++;
		  }
...
/*		 str = sprintf("%s%12s %s\n", str, RANK_D->query_rank(list[j]),
	...
	// Skip those users in login limbo.
		 if( !environment(list[j]) ) continue;
		 str = sprintf("%s%-15s%s", str,
				opt_id ? capitalize(list[j]->query("id")): (query_idle(list[j]) > 120?""HIC"*"NOR""+list[j]->name(1):list[j]->name(1)),
				(ppl_cnt%5==4? "\n": "")
/*		 str = sprintf("%s%-15s%s", str,
