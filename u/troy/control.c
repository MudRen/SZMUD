...
int do_start(string arg)
{
	...
	message_vision(HIR"$N宣布试剑山庄现在开放！\n"NOR,me);
	CHANNEL_D->do_channel(me, "chat", HIR"宣布试剑山庄现在开放！\n"NOR);
	CHANNEL_D->do_channel(me, "chat", HIY"请大家从扬州客店入场（enter->northup)！\n"NOR);
	set("bwdh",1);
	return 1;
}
...
int do_stop(string arg)
{
	...
	delete("bwdh");
	message_vision(HIR"宣布试剑山庄关门喽！\n"NOR,me);
	CHANNEL_D->do_channel(me, "chat", HIR"宣布试剑山庄关门喽！\n"NOR);
	return 1;
}
...
int do_jie()
{
 	 ...
 	 object board =  find_object(BOARD);
	 board->delete("team");
	 board->delete("counts");
	 board->delete("east");
	 board->delete("west");
