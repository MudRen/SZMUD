...
int convert(string arg)
{
	...
	message_vision("$N"+HIW"看了看$n，突然似箭一般跃起，准确地落在蛇杖上，盘了起来。\n"NOR,
		snake,me);
	if ( me->query("bt/job") != 0){
		if ((me->query("bt/exp")+me->query("bt/pot")+me->query("bt/lvl")) != 0)
			log_file("/job/bt-fangshe",sprintf("[%s] %s(%s) 从 %d 秒的放蛇[%s条]工作中取得 %d 点经验 、 %d 点潜能 和 %d 点驱兽术\n", ctime(time()),me->name(), me->query("id"),time()-me->query("bt/time"),chinese_number(me->query("bt/job")),me->query("bt/exp"),me->query("bt/pot"),me->query("bt/lvl")));
		me->delete("bt", 0);
	}
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	...
	if( (owner=present(me->query("owner"),here)) )
	{
		...
		owner->add("combat_exp", exp);
		owner->add("potential", pot);
		owner->add("bt/lvl", lvl);
		owner->add("bt/exp", exp);
		owner->add("bt/pot", pot);		
...
int auto_perform()
{
	...
	int damage,bonus;
	int exp, pot, lvl;
    int zero_exp = target->query("combat_exp");
