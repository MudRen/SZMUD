...
void init()
{
  ...
  if ( me->query("job/supervisor") && !( query_temp("worker/"+ me->query("id"))) )
  	clear_guard_job(me);
...
int do_volunteer()
{
  ...
  if ( query_temp("stock") > 2)
    {
      message_vision(CYN"知客道长对着$N说道：你来晚了，今天的活儿我都分派完了。\n"NOR,ob);
      return 1;
    }
...
void end_volunteer(object who, int jobid)
{
  ...
  if ( !objectp(who) )
  {
    if (zhike->query_temp("stock") > 0)  zhike->add_temp("stock",-1);
    return;
  }
  
    
  if ( !present(who, environment(zhike)) 
  ||(string)who->query("job/description") != "守卫山门"  
  ||  (int)who->query("job/id") != jobid ) 
  {	
  	if ( query_temp("worker/"+who->query("id")) )
  	{ 	if ((query_temp("stock") > 0) && query_temp("worker/"+who->query("id")) ) add_temp("stock",-1);
    		delete_temp("worker/"+who->query("id"));
    	}    	
    	clear_guard_job(who);
     	return;
  ...
  if (query_temp("stock")>0) add_temp("stock", -1);
  clear_guard_job(who);
