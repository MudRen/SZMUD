...
void remove_effect1(object me, int amount);
...
int exert(object me, object target)
{
        ...
        else if( me->query_skill("zixia-gong",1) < 350 && (me->query_temp("zixia/lvl") == 1 || me->query_temp("zixia/lvl") == 3))
                return notify_fail("你已经施展出紫霞神功了。\n");
        ...
        ******************************************|Modify yun zixia by more mode|Modify by Karlopex@sz ****/
        if( !weapon ){
                if( !me->query_temp("zixia/lvl",1) ){
                        data = sprintf("attack ==> %d  damage ==> %d  dodge ==> %d  strength ==> %d  dexerity ==> %d ", me->query_temp("apply/attack"), me->query_temp("apply/damage"), me->query_temp("apply/dodge"), me->query_temp("apply/strength"), me->query_temp("apply/dexerity"));
                        ...
                        me->start_call_out((:call_other,__FILE__,"remove_effect1", me, skill:),skill);
                }else{
                        data = sprintf("attack ==> %d  damage ==> %d  dodge ==> %d  strength ==> %d  dexerity ==> %d ", me->query_temp("apply/attack"), me->query_temp("apply/damage"), me->query_temp("apply/dodge"), me->query_temp("apply/strength"), me->query_temp("apply/dexerity"));
...
void remove_effect1(object me, int amount)
{
        if(!me || !me->query_temp("zixia") ) return;
        ...
        if(me->query_temp("zixia/lvl") == 2){
                ...
                if(wizardp(me)){
                        data2 = sprintf("attack ==> %d  damage ==> %d  dodge ==> %d  strength ==> %d  dexerity ==> %d ", me->query_temp("apply/attack"), me->query_temp("apply/damage"), me->query_temp("apply/dodge"), me->query_temp("apply/strength"), me->query_temp("apply/dexerity"));
...
void remove_effect2(object me, int amount)
{
        if(!me || me->query_temp("zixia/lvl") != 2) return;
        ...
        if( me->query_temp("zixia/remove") != 1){
                ...
                if(wizardp(me)){
                        data2 = sprintf("attack ==> %d  damage ==> %d  dodge ==> %d  strength ==> %d  dexerity ==> %d ", me->query_temp("apply/attack"), me->query_temp("apply/damage"), me->query_temp("apply/dodge"), me->query_temp("apply/strength"), me->query_temp("apply/dexerity"));
                        tell_object(me, HIG"\n\n运二段紫霞前的数据：　　"+me->query_temp("zixia/data2")+"\n运紫霞第二阶段单独收功的数据：　　"+data2+"\n\n"NOR);
                }
                if (me->is_fighting())
...
void remove_effect_weapon1(object me, object weapon, int amount)
{
        ...
        else weapon->add("weapon_prop/damage", -amount / 4);
...
void remove_effect_weapon2(object me, object weapon, int amount)
{
        if(!me || me->query_temp("zixia/lvl") != 4) return;
