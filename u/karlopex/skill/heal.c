...
int curing(object me)
{
	...
	if(mode == 1){
		...
		{
			if( !msg || undefinedp(msg["finish_healother"]) ){
				...
				tell_object(me, msg["finish_healother"]["me"]);
				tell_object(target, msg["finish_healother"]["target"]);
			}
					
			if( !msg || undefinedp(msg["finish_healother_other"]) )
				message("vision", me->name() +"感觉"+target->name()+"的气息均匀，内伤已经痊愈，便收起内功站了起来。\n"NOR,	environment(me), me);
			else	message("vision", msg["finish_healother_other"], environment(me), me);
		...
		{
			if( !msg || undefinedp(msg["unfinish_healother"]) ){
				...
				tell_object(me, msg["unfinish_healother"]["me"]);
				tell_object(target, msg["unfinish_healother"]["target"]);
			}
				
			if( !msg || undefinedp(msg["unfinish_healother_other"]) )
				message("vision", me->name() +"运功完毕，缓缓站了起来，"+target->name()+"的脸色看起来好了一些。\n"NOR,environment(me), me);
			else	message("vision", msg["unfinish_healother_other"], environment(me), me);
		...
		effect = lvl * max/10 * eff / (2*lvl + lvl*con*2/30) / 20;
...
int halt_curing(object me)
{
	...
	if(mode == 1){
		...
		tell_object(me, "你在"+target->name()+"耳边轻声说道：不用为我疗伤了。说完你侍"+target->name()+"内功收回后缓缓站了起来。\n");
