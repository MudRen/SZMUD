...
void create()
{
        set_name("【神州】电子新闻版", ({ "newsboard"}) );
        set("location", "/d/wizard/station_room"); // 这里用了一个ROOM来放置BOARD，简单，你做
        set("board_id", "newsboard");              // 成一个指令来输入新闻也可以。
        setup();
        set("capacity", 500);
}
...
int check_news(object me)
{
        ...
        if(userp(me))
        {
                ...
                if(i>10) write("你可以 Help news 来查看【神州】新闻系统的使用方法。\n"); // 提醒不知道有NEWS系统的玩家使用news
        }
}
...
int main(object me, string arg)
{
        ...
        if( !pointerp(notes) || !sizeof(notes) )
                return notify_fail("「【神州】」目前没有任何新闻。\n");

        if( !arg )
        {
                if(i>10) write("你可以 Help news 来查看【神州】新闻系统的使用方法。\n");
                        return notify_fail(
                                "「【神州】」目前共有"+HIW+chinese_number(sizeof(notes))+NOR"条新闻"+((i==0)? "。":"，您有"+HIY+chinese_number(i)+NOR"条新闻尚未阅读。")+"\n"); }
        ...
        "-------------------------------------------------------------------------------\n",
                BBLU HIG"「"HIG"【神州】"HIG"」第"+chinese_number(num + 1)+"条新闻"NOR,
...
void done_post(object me, mapping note, int re, string text)
{
        mapping *notes;
