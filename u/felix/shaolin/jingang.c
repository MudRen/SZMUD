...
*/
        ...
        if( me->query_skill("jingang-quan", 1) < 90 )
		  ...
		  if( me->query_temp("jinzhongzhao") )
					 return notify_fail("你正在运用金刚不坏体神功，功力未够，无法同时运用金刚伏魔神通。\n");

		  if( me->query_temp("huaying") )
			  return notify_fail("你正在施展分身化影，无法同时运用金刚伏魔神通。\n");

		  if( me->query_temp("dujiang") )
			  return notify_fail("你正在施展一苇渡江，无法同时运用金刚伏魔神通。\n");
