...
int Get_Success_Rate(float f_My_Level, float f_Target_Level, float f_Basic_Success_Rate, float f_Add_Success_Rate, float f_Skill_Level_Diff)
{
	...
	if(f_My_Level >= f_Target_Level)
	{
		...
		f_Success_Rate = f_Basic_Success_Rate * pow(to_float(1 + f_Add_Success_Rate), to_float((f_My_Level - f_Target_Level) / f_My_Level / f_Skill_Level_Diff));
	}

	// If performer's skill/exp is less than target
	if(f_My_Level < f_Target_Level)
	{
		f_Success_Rate = f_Basic_Success_Rate * pow(to_float(1 - f_Add_Success_Rate), to_float((f_Target_Level - f_My_Level) / f_Target_Level / f_Skill_Level_Diff));
	}
