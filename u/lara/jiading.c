...
int do_recognize(string arg)
{
	...
	if ( mapp(skill_status = me->query_skills()) ) {
		...
		ob->set("gain_exp", gain*11/10);      //提高10%的报酬
		ob->set("gain_pot", gain/3 +random(gain/10));//提高得潜能的比率
		...
		ob->set("jiali", me->query("jiali")/3);//降低加力1/2->1/3
		if ( me->query("max_neili") > 0 ) {
			ob->set("max_neili", level*20); //本来*30是满值了，现在改为20
	...
	message_vision(HIR"胡人拍了拍手，身後突然跃出"+chinese_number(num)+"个铁甲护卫，"+chinese_number(num+1)+"人向$N左右包抄！\n"NOR, me, ob);
		...
		huwei->set_leader(ob);
		me->fight_ob(huwei);
		ob->add("gain_exp", random(10)+30);//提高此处的报酬
		ob->add("gain_pot", random(5)+15); //提高此处的报酬
		}
	}
	...
	ob->set_skill("poison", level/3);//毒性减为1/3
...
int do_bring(string arg)
{
	...
	if(yapu_exp > 600) yapu_exp = 600+random(40); //提高100上限
	if(yapu_pot > 300) yapu_exp = 300+random(30); //提高50上限
	...
	me->set("th_yapu_ok",1);
		...
		me->set("th_exp",50000);
		tell_object(me,"你的桃花经验已经达到50000点的满点，不能再累积了。\n");
		}//提高5倍

		if(me->query("th_pot")>me->query("max_potential")*30) {
		me->set("th_pot",me->query("max_potential")*30);
		tell_object(me,"你的桃花潜能已经达到"+(me->query("max_potential")*30)+"点的满点，不能再累积了。\n");
		}//提高3倍
