...
int do_divine(string arg)
{
	...
	{ 
		...
		{
me->add("combat_exp", rate/2+random(me->query("int")+me->query_int()-40)/2);
if (me->query("potential")<me->query("max_potential")) me->add("potential",rate/2+random(me->query("int")+me->query_int()-40)/4);//调整报酬计算公式与先后天悟性有关
		}
	...
	else if( me->query_int() > random(ob->query_kar()) )
	{
		...
		if( me->query("combat_exp") < (me->query("int")*3+me->query_int()) * 5000 )
		{
me->add("combat_exp", rate/3+random(me->query("int")+me->query_int()-40)/3);
			if (me->query("potential")<me->query("max_potential")) me->add("potential",rate/3+random(me->query("int")+me->query_int()-40)/4);
		}//调整报酬计算公式与先后天悟性有关
