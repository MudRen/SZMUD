...
{      
    ...
    if( me->query_temp("obj/songxin")>0)
    {
                tell_object(me,CYN"何足道对你冷哼："RED"没用的家伙，还不给我滚!"CYN"了。\n"NOR);
                return 1;
    } 
    else 
      if( me->query_skill("qinqi-shuhua")<120)
    {
                      tell_object(me,"何足道看着你摇了摇头说道: 我考虑考虑。\n"); 
                return 1;
    }

    else
        if(me->query("family/family_name")!="昆仑派" && me->query("combat_exp")>=100000)
        {
               message_vision("何足道对$N作了一揖道：阁下是不是开玩笑?\n",me);
        return 1;
        }
    message_vision(CYN"何足道对$N说道：你去少林给觉远大师带个口信：就说书在什么包里面。\n记住一定要小心，不要在中原给我丢脸，你去吧。\n"NOR,me); 
    
        me->set_temp("obj/songxin", 1);
        return 1;   
}
