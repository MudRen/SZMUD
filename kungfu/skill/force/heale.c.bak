...
int exert(object me)
{
	...
	me->set("jiali", 0);
        write(HIG "你现在疗伤进度：" + process_bar((me->query("eff_qi") + 1) * 100 / me->query("max_qi")) + "\n");
	me->attach_system();
	me->write_prompt();
	return 1;
}
...
int curing(object me)
{
	...
	message("system", ESC + "[1A" + ESC + "[256D"
		HIG "你现在疗伤进度：" + process_bar((me->query("eff_qi") + 1) * 100 / me->query("max_qi")) +
		"\n" + (me ? HIR "疗伤中" NOR "> " : "") + (wizardp(me) ? "每次治愈"+chinese_number(effect)+"点内伤。" : "" ),me);
	me->add("neili", -50);
	return 1; 
}
