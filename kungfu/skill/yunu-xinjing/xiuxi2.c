...
int exert(object me, object target)
{
	...
	else
	{
		...
		me->set_temp("pending/xiuxi", 2);
		me->start_busy((: xiuxi :), (:halt_xiuxi:));
		target->set_temp("pending/xiuxi", 2);
...
int xiuxi(object me)
{
	...
	//伙伴停就停
	if( target->query_temp("pending/xiuxi") != 2 )
	{
		...
		&& 	((xj_lvl>270) || ((xj_lvl+1)/30*30 != xj_lvl+1)))
	{
		...
		me->add("combat_exp",1);
/*
		f = to_float(target->query("combat_exp"))/to_float(me->query("combat_exp"));
		if (f>=2) f=2;
...
*/
		...
		else if (me->query("gender") != target->query("gender")) xj_gain *= 1.2;
