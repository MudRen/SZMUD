...
/*        if( objectp(target) && target != me ) 
	...
	if( !me->query_temp("zixia/exert",1) ){
		tell_object(me, MAG"\n你气凝丹田，运起紫霞神功，脸上紫气大盛。\n"NOR);
		message("vision", MAG"\n\n突然之间，"NOR+me->name()+MAG"脸上满布紫气，只是那紫气一现即隐，顷刻间又回复了原来脸色。\n\n" NOR, environment(me), ({me}) );
		me->set_temp( "zixia/lvl", skill / 100);
		me->add_temp( "zixia/exert", 1);
                ...
                me->add_temp( "apply/force", skill / 10 );
...
//                me->start_call_out((:call_other,__FILE__,"remove_effect", me, weapon, "nomarl":), skill);
        	call_out("check", 1, me, weapon);
                ...
                if( !me->is_fighting() || me->query_temp("zixia/exert") >= 2 )
...
{
        string wieldw;
        if(!me || !me->query_temp("zixia/exert")) return 1;
        ...
        if( me->query_temp("zixia/exert") >= 2 ) {
                me->set_temp("zixia/exert", 1);
        }
        return 1;
}
...
void check(object me, object weapon)
{
	int poi_lvl, poi_amount, zx_lvl, cure, zx, skill;
        string myskill_type, stat;
        mapping pmap;
        if(!me || me->is_ghost() || !living(me) || !me->query_temp("zixia/exert")){
		if ( !living(me) || me->is_ghost() )
			remove_effect(me, weapon, "death");
		return;
        }
	skill = (int)me->query_skill("zixia-gong", 1)+((int)me->query_skill("ziyin-yin", 1)+(int)me->query_skill("zhengqi-jue", 1))/2;
	zx = me->query_temp("zixia/add_lvl",1);
	zx_lvl = (int)me->query_skill("force");
        ...
        if( me->query_skill_mapped("force") != "zixia-gong" ){
		message_vision(CYN "\n$N脸色十分苍白，看来$N身上的紫霞真气与内功心法相悖，$N只好强行将紫霞真气收回！\n"NOR, me);
	...
	if (me->is_fighting()){
		...
		if ( zx % 10 == 0 ){
	                ...
	                me->add_temp( "apply/damage", skill / 100 );
	                me->add_temp( "apply/dodge", skill / 100 );
	                me->add_temp( "apply/speed", skill / 100 );
		}
		if ( zx > 25 ) zx = 25;
		me->add("neili", -(20 + random(zx)));
		me->add_temp("zixia/add_lvl" , 1);
		if ( wizardp(me) ) tell_object(me, MAG"zixia/add_lvl add 1   ......   "HIR"OK!\n"NOR);
	}else{
		me->set_temp("zixia/add_lvl" , 1);
		me->add("neili", -( me->query_temp("zixia/lvl", 1) / 5 + random(10)));
	}
        call_out("check", 1, me, weapon);
}
