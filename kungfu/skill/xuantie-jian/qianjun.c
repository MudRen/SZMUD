...
int perform(object me, object target)
{
        ...
        if( me->query_skill("force") < 300 )
                return notify_fail("你的玉女心经火候未到，无法施展千均一击！\n");

        if( me->query_skill("sword") < 300 )
                return notify_fail("你的玄铁剑法火候未到，无法施展千均一击！\n");
        ...
        if( me->query("jingli") <= 1000 )
         ...
         ap=me->query("combat_exp")/900+me->query_skill("force")+me->query_skill("sword");
         dp=target->query("combat_exp")/1000+target->query_skill("dodge");
              damage=me->query("combat_exp")/850 +me->query("jiali")*2+me->query("jiajing")*2; 
         if (damage<3000) damage=2000+random(500);
         if (damage>12500) damage=12500+random(2000);
