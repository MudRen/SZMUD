...
int perform(object me, object target)
{
        ...
        ||      !me->is_fighting(target) )
                return notify_fail("「戚然消魂」只能在战斗中使用。\n");

        if(me->query("spouse/id"))
                return notify_fail("你有娇妻陪伴左右，终是领悟不到「戚然消魂」其中的妙境。\n");

        if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
                return notify_fail("「戚然消魂」只能空手使用。\n");

        if( (int)me->query_skill("yunu-xinjing", 1) < 150 )
                return notify_fail("你的玉女心经修为不够，不会使用「戚然消魂」。\n");

        if( me->query_skill_mapped("force") != "yunu-xinjing" )
                return notify_fail("你所用的并非玉女心经，使不出「戚然消魂」！\n");
        ...
        if( (int)me->query_skill("xiaohun-zhang",1) < 150 )
                return notify_fail("你的戚然消魂掌火候不够，无法使用「戚然消魂」！\n");

        if( jiali < 50 )
         return notify_fail("「戚然消魂」的威力全系于个人内力，你掌上不带内力怎能使出？\n");
        ...
        msg += WHT "$N大喝一声：“戚然消魂掌”！所有的悲愤与哀怨随掌劲倾泻而出，顿时天地间一片苦涩！\n"NOR;
