...
int perform(object me, object target)
{
 	...
 	if( me->query_skill_mapped("sword") != "quanzhen-jian" )
        ...
        if (me->query("quanzhen/lh_count")>60)
        	me->set("quanzhen/lh_count",1);
        
	count = me->query("eff_qi") * 100 / me->query("max_qi");
        me->add("neili", -350);
        me->add("jingli",-250);
        me->start_busy(3);
                        
        limit=600;
        if (count > 80) tell_object(me, HIG"\n\n你精神抖擞，内力充沛，出招格外凌厉。\n\n"NOR);
        if (count > 40 && count <= 80) {
        	tell_object(me, HIG"\n\n你受了轻伤，招数已经没有以前凌厉，担仍然咄咄逼人。\n\n"NOR);
        	limit=300;
        }
        if (count < 40) {
        	tell_object(me, HIG"\n\n你的伤势过重，〖三连环〗的招数已经失去了原有的凌厉。\n\n"NOR);
        	limit=150;
        }
        ...
        if (addon>limit *3/2) addon=limit *3/2;
