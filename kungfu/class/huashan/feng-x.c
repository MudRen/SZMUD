...
int improve_sword(object ob)
{
	...
	if (add_level >= 1) tell_object( ob, "你的基本剑法增加了"+chinese_number(add_level)+"级！\n");
...
void attempt_apprentice(object ob)
{
	...
	if( objectp(ob1 = present("mengzhu", room)) )
        ...
        if ( ob->query("family/family_name") && ob->query("family/family_name") != "华山派" ) {
                command("say "+RANK_D->query_respect(ob)+"，即是"+ob->query("family/family_name")+"的弟子，又为何要投入华山门下？");
                command("say 如此不忠之徒，恕我华山派不能收留！");
		return;
	}
...
/*	if ( (int)ob->query_skill("taiji-jian") > 0 || (int)ob->query_skill("damo-jian") > 0
	...
	log_file("/bai_feng",sprintf("[%s] %s(%s) 拜 风清扬 为师", ctime(time()),me->name(), me->query("id")));//add by lisser 
