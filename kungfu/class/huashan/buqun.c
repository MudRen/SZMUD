...
//        if ( ob->query("huashan/job_pending") && !ob->query_condition("hz_job") ) {
		...
		if ( ob->query("family/family_name") != "华山派" ){
	                if ( sizeof(job_stat) < 10 ) {
	                        ...
	                        message_vision("$N皱着眉头对$n说道：华山很大，请"+RANK_D->query_respect(ob)+"各处好好巡视一下。\n", me, ob);
	                }
	                else 
	                {
	                        ...
	                        command("say 很好，"+RANK_D->query_respect(ob)+"将来必定成为武林的栋梁。");
	                }
	        }else{
...
string ask_job()
{
        ...
        if ( ob->query("scheming")) return "现在禁止用 Scheme 做门派工作！\n"; 
		...
		if ( time() < ob->query("mp_job_time") + BUSY_TIME ) 
			return "这位"+RANK_D->query_respect(ob)+"，这里不缺人手，请到其他地方看看。\n";
	        ob->set("huashan/job_pending", 1);
		ob->set("mp_job_time",time());
                return "好吧！"+RANK_D->query_respect(ob)+"，请你仔细巡察华山各处，多有劳烦了。\n";
	}

	if ( time() < ob->query("mp_job_time") + 180 ) 
		return "现在有太多的人在巡山，你还是先到练功房里打坐练功吧。\n";
