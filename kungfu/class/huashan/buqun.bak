...
int check_student(object ob)
{
        ...
        if ( ob->query("huashan/job_pending") && !ob->query_condition("hz_job") ) {
                ...
                //log_file("job/huashan",ob->query("name") +" (" + ob->query("id") + ") 巡山获得 " + 
            //    expG + " 点经验和 "+ potG + " 点浅能 "+ctime(time())+"\n");
...
string ask_job()
{
        ...
        if(ob->query("huashan/robotb") >= 1) return "你还要等待的时间是" + ob->query("huashan/robotb") + "秒";

        if ( ob->query_condition("hz_job") > 0 && sizeof(job_stat) == 12 )
                return "你没仔细巡山吧？这么快便巡遍了华山？给我老老实实地回去巡山吧！\n";
        else if ( ob->query_condition("hz_job") > 0 )
                return "你还没完成我交待的工作，怎么又来问我？\n";
        ...
        if ( obj->query_temp("owner") == ob->query("id") ) 
        {
				   ...
				   remove_call_out("fail_question_count"); 
				     call_out("fail_question_count", 1); 
                                tell_object(ob, "你的等待时间是" + ob->query("huashan/robotb") + "秒。\n");
				destruct(obj);
                     return 1;
			}
                        if( random(100) < 50)
                ...
                message_vision("$N向$n置问道：这令牌似乎不是你的。你是怎么得来的？\n", me, ob);
        }
...
int do_bonus()
{
 ...
 // Increase hs job rate from 100 to 400
 // by buwu. 加了机器人检查以后, 提高收益到武当面壁同一水平. to 700
    rate = 700;
    exp = ob->query("combat_exp");
        ...
        command("say "+ob->name()+"你做得非常好！快下去休息吧。");
                // reduce the radom rate from 50 + 50 to 80 + 20
                expG = rate*(80+random(20))*intPaiValue/100;
                ob->add("combat_exp", expG);
                potG = rate*(30+random(20))*intPaiValue/100;
                ob->add("potential", potG );
                if ( ob->query("potential") > ob->query("max_potential") )
                    ob->set("potential", ob->query("max_potential"));
        
                                
                                log_file("job/huashan",ob->query("name") +" (" + ob->query("id") + ") 巡山杀MMR获得 " + expG + " 点经验和 "+ potG + " 点浅能 "+ctime(time())+"\n");
...
int do_answer(string arg)
{
        ...
        if(result == -1) {
                ...
                remove_call_out("fail_question_count"); 
                call_out("fail_question_count", 1); 
                tell_object(user, "蒙面人是高收益的工作，请不要再使用机器人。\n(你的等待时间是" + user->query("huashan/robotb") + "秒。)\n");
