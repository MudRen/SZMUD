// MODIFY BY SCATTER
// V. 1.1
// by snowy 2002/1  【巫师】布武(Buwu)：+shen biaoju job 是 400 + random(100) exp
...
//              if( ob->query("family/family_name") == "华山派" ) bonus = bonus - 150; // 华山派 ppl can get extra bonus from xuanshan
        	...
        	bonus = bonus*2;
                ...
                add_temp("biao", -1);
                log_file("/test/BiaoJob", ctime(time()) + ob->query("name") +"(" + ob->query("id") + ")在福威镖局保镖获得" + addexp + "经验和"+ (ob->query("potential")-old_pot) + "潜能\n" );
        }
...
int ask_fuwei()
{
        say("林震南炫耀地说：我们林家三代走镖，一来仗着先祖远图公当年闯下的威名，二\n"
                "来靠着我们林家家传的玩艺儿不算含糊，这才有今日的局面，成为大江以南首屈\n"
...
int ask_pixie()
{
        ...
        else if ((int)this_player()->query_temp("marks/林2"))
        {
               ...
               p1=random(4)+1;
                ...
                CHINESE_D->chinese_number(p1)+"千"+CHINESE_D->chinese_number(p2)+
...
int do_jobwith(string arg)
{
		...
		return notify_fail( "这桩差事很辛苦，你还是多休息休息，过"+ chinese_number(900 - time() + ob->query("bj_job_time")) +"秒再来问吧。\n" );		
       	if ( time() < ob->query("bj_job_time") + 720 && ob->query("combat_exp") > 200000 )
		return notify_fail( "这桩差事很辛苦，你还是多休息休息，过"+ chinese_number(720 - time() + ob->query("bj_job_time")) +"秒再来问吧。\n" );
       	if ( time() < ob->query("bj_job_time") + 600 && ob->query("combat_exp") > 2000000 )
		return notify_fail( "这桩差事很辛苦，你还是多休息休息，过"+ chinese_number(600 - time() + ob->query("bj_job_time")) +"秒再来问吧。\n" );
       	if ( time() < partner->query("bj_job_time") + 900 && partner->query("combat_exp") < 200000 )
		return notify_fail( "这桩差事很辛苦，你的搭档还需要休息一下，过"+ chinese_number(900 - time() + ob->query("bj_job_time")) +"秒再来问吧。\n" );	
       	if ( time() < partner->query("bj_job_time") + 720 && partner->query("combat_exp") > 200000 )
		return notify_fail( "这桩差事很辛苦，你的搭档还需要休息一下，过"+ chinese_number(720 - time() + ob->query("bj_job_time")) +"秒再来问吧。\n" );
       	if ( time() < partner->query("bj_job_time") + 600 && partner->query("combat_exp") > 2000000 )
		return notify_fail( "这桩差事很辛苦，你的搭档还需要休息一下，过"+ chinese_number(600 - time() + ob->query("bj_job_time")) +"秒再来问吧。\n" );
        ...
        {
                command( "hmm" );
                return notify_fail("你武功这么差，怎么配在本镖局任事？\n");
        }
...
string ask_job()
{
        ...
        if ( ob->query("biao/fail") )
        {
       	...
       	if ( time() < ob->query("bj_job_time") + 900 && ob->query("combat_exp") < 200000 )
		return "这桩差事很辛苦，你还是多休息休息，过"+ chinese_number(900 - time() + ob->query("bj_job_time")) +"秒再来问吧。\n";		
       	if ( time() < ob->query("bj_job_time") + 720 && ob->query("combat_exp") > 200000 )
		return "这桩差事很辛苦，你还是多休息休息，过"+ chinese_number(720 - time() + ob->query("bj_job_time")) +"秒再来问吧。\n";
       	if ( time() < ob->query("bj_job_time") + 600 && ob->query("combat_exp") > 2000000 )
		return "这桩差事很辛苦，你还是多休息休息，过"+ chinese_number(600 - time() + ob->query("bj_job_time")) +"秒再来问吧。\n";
        if ( ob->query_condition("biao") )
        {
