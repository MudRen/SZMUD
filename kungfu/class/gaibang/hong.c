...
string ask_wugou()
{
       ...
       if (me->query("family/master_name")!="洪七公")
        ...
        return RANK_D->query_respect(this_player())+"还未为我帮立下大功，这个，这个，不能轻传啊!";
       ob=new("/u/slow/questwg/dgb_book");
       if (present("dgb book",me)) return "咦，我不是给了你了么";
        if (!ob) return "嗯，借给你倒是没问题，只是刚被别的弟子借走了，你过一阵再来问问吧。";
        ob->set("master",me->query("id"));
        ob->move(me);
        message_vision("$n递给$N一本打狗棒谱。\n",me,this_object());
        command ("rumor "+me->query("name")+"弄到了打狗棒谱。");
        return "记住：娃儿，要用口诀配合着打狗棒谱来领悟(lingwu)啊。";
}
