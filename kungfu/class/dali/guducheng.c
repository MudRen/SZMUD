...
string ask_me1()
{
	...
	if (environment()!=room)
		return "对不起，软甲都在库房里，我没带出来，你等我回去再来问我要吧。\n";

	if (room->query("ruanjia_count") >= 5)
		return "软甲乃我大理国宝，现在已经被别人拿光了。";

	if (ob=present("ruanjia", who))
		return RANK_D->query_respect(who)+"你现在身上不是有软甲吗，你一个人能穿几件啊？";

	log = room->query_temp("ruanjia");
	if (mapp(log) && log[getuid(who)])
		return "你刚才不是来领过软甲了吗，怎麽又来要了？";

	if (who->query("dali/rank")<4)
		return "你的级别不够，不能领取软甲，还是在库房里找点其它的用吧。\n";
	...
	return "拿去吧，这软甲乃我大理国宝，你要小心使用，努力报国。";
}
void init()
{
	object ob;
