...
string ask_qingyan()
{
	...
	if (me->query("family/family_name")!="大理段家")
	            return  RANK_D->query_respect(me)+"并非我大理臣子!";
	                                
         if ((int)me->query("dali/rank")<=1)
	            return  RANK_D->query_respect(me)+"是否为大理国出力太少了!";
	                  
	  if (me->query("dali/qingyan")==1)
	            return  RANK_D->query_respect(me)+"已经学会五罗轻烟，不要无理取闹!";
	      
	  
	    me->set("dali/qingyan",1);
	    message_vision("段正淳站起身来，对$N慢慢讲解五罗轻烟掌中的奥秘！\n",me);
            return "此乃我大理段家绝学，望你以后能为大理国造福！";
	    }
