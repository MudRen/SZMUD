...
int do_volunteer()
{
  ...
  {
             tell_object(ob,CYN"你才刚做完你不累呀，我看还是休息一下让别人做吧。\n"NOR); 
	  return 1; 
  } 

         if ( ob->query("scheming") == 1 )  
        {
           tell_object(ob,CYN"灵虚道长说道：“现在禁止用 Scheme 挂机做工作。”\n"NOR); 
        return 1; 
} 
  ...
  if ( ob->is_busy() || ob->is_fighting() )
    {
      ...
      tell_object(ob,"你想自愿为知客道长守卫山门。\n"); 
                ob->set("wdvol_job_time",time() );
...
{
object obn, zhike=this_object(), obinc;
  int i, j, rate;
  ...
  if ( (who->query("family/family_name") == "武当派" ))
    {
                ...
                who->add("potential", 60 + random(40));
	}
...
this_player()->add("wudang/offerring", random(2));
    
if(this_player()->query("wudang/offerring") > 7)
{
   this_player()->add("wudang/offerring", -3);
   obinc = new("/d/wudang/obj/incense");
   obinc->move(this_player());
   command("say и硂柑Τ穛");
}
