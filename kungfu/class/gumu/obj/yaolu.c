...
int do_addyao(string arg)
{
	...
	if (!obj->query("yaocai")) return notify_fail("药炉里只能放用以炼制的药材。\n");
...
int do_pour(string arg)
{
	...
	if ( arg == "玉蜂毒" ){
		...
		add("cure_d",0);
		add("cure_n",-80);
		message_vision("$N从"+box->query("name")+"中抓了几只玉蜂，滴了几滴玉蜂毒在药炉里。\n",me);
		return 1;
	}
	...
	message_vision("$N从"+ping->query("name")+"中倒了一些"+arg+"在药炉里。\n",me);
	return 1;
}
...
int do_aoyao()
{
	...
	if ((int)me->query("neili") < 500) return notify_fail("你的内力不足！\n");

	if ((int)me->query("jingli") < 1000) return notify_fail("你的精力不足！\n");
