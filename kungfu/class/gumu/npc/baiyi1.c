...
string ask_me()
{
	...
	me->set_skill("yunu-xinjing", 30);
	me->delete_skill("quanzhen-xinfa");
	me->delete_skill("yunu-xinfa");
	if ( !me->query("has_xinjing") ) {
		this_player()->add("max_neili" ,30);
		me->set("has_xinjing",1);
	}
	command("congra " + me->query("id"));
	command("say " + RANK_D->query_respect(this_player()) +
		"的古墓武功大有精进！");
	return RANK_D->query_respect(this_player()) +"恭喜你！";
}
