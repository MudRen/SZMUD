...
#define SIFANG "/d/forest/sifang/sifang4_file"
#define DDR "/d/forest/"
#define CL_POS "鹰迅堂"
...
void init()
{
	...
	string pos;
        ...
        if (!stringp (pos = me->query("clbpos")) && (me->query_temp("invite")!= CL_POS))
...
int gohome( string arg )
{
    string id = this_player()->query("id");
    if ( id == "bye" || id == "qingl" )
        this_player()->move( "/p/fystreet/room/qingl_room" );
    return 1;
}
