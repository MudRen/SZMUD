...
#define SIFANG "/d/forest/sifang/sifang2_file"
#define DDR "/d/forest/"
...
void init()
{
	...
	string pos;
        ...
        if (stringp (pos = me->query("clbpos")) && (pos == "狮敏堂")) {add_action("do_dazuo","dazuo");add_action("do_dazuo", "exercise"); }
...
int do_invite(string arg)
{
	...
	if ( !stringp(me->query("clbpos")) || me->query("clbpos")!="狮敏堂")
	...
	&& userp (ob) )
	{	
		ob->set_temp("invite","狮敏堂");
		write("已经帮你邀请了"+ob->query("name")+"。\n");
		tell_object(ob,HIM"【邀请】：" + "长乐帮狮敏堂香主 " + me->query("name")+"邀请你去他的香主私房。\n"NOR);
		return 1;
	}
