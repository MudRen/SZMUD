...
int do_zhuangxiu()
{
   ...
   if(this_player()->query("balance")<280000) return notify_fail("你的存款还不够二十八两黄金。\n");
    builder = new("/p/fystreet/npc/builder");
    builder->move(this_object());
    message_vision(CYN"$N说道：请问要修成什么样子的？\n"NOR, builder);
...
{  string file,filename,bakname,long_old;
   if(arg[0..0] != "y") {
   message_vision(CYN"$N说道：既然不愿意修那也就算了！\n"NOR, present("room builder", this_object()));
   message_vision("$N离开了。\n", present("room builder", this_object()));
   destruct(present("room builder", this_object()));
   return;
   }
   ...
   long_old = this_object()->query("long");
   file=replace_string(file,long_old,text);
   if(!write_file(filename,file,1)) write("write file fail.\n");
   this_object()->set("long",text);
   message_vision("$N将房间好好装修了一番。大功告成！\n",present("room builder", this_object()));
      this_player()->add("balance", -280000);
   message_vision("$N离开了。\n", present("room builder", this_object()));
   destruct(present("room builder", this_object()));
return;}
...
int do_yaoqing(string arg)
 {
  ...
  tell_object(ob,room->query("short")+"就在神州大街"+chinese_number(roomid/100)+"号"+((roomid%2)?"西":"东")+"。\n");
  return 1;
 } 
